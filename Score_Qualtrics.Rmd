...
packages <- c("lme4", "nlme", "ggplot2", "zoo","dplyr", "tidyr", "knitr",
              "parallel", "data.table", "lubridate","qualtRics")
...
```{r, include=FALSE}
tagqualtricstoken<-readChar("/Volumes/psych-cog/dsnlab/TAG/behavior/Questionnaires/Confidential/qualtrics_TAG_token.txt",nchars = file.info("/Volumes/psych-cog/dsnlab/TAG/behavior/Questionnaires/Confidential/qualtrics_TAG_token.txt")$size) 
qualtRics::registerApiKey(tagqualtricstoken)
current_surveys <- qualtRics::getSurveys("https://oregon.qualtrics.com/")

sess1_surveys<-current_surveys %>% filter(grepl("Sess 1",name))
home_surveys<-current_surveys %>% filter(grepl("Home",name))
sess2_surveys<-current_surveys %>% filter(grepl("Sess 2",name))
parent_surveys<-current_surveys %>% filter(grepl("Parent",name))

surveys_to_process<-bind_rows(sess1_surveys,sess2_surveys,home_surveys,parent_surveys)

for (i in 1:length(surveys_to_process$id)){
getSurvey(surveys_to_process$id[i],
                      format = "csv",
                      save_dir="/Volumes/psych-cog/dsnlab/TAG/behavior/Questionnaires/Working/",
                      "https://oregon.qualtrics.com/",
                      verbose=TRUE)}
...
```{r}
qualtrics_test<-merge(survey)




qualtricsData_raw_long <- read.csv(paste0(processed_data,"qualtrics/TAG_auto_raw_qualtrics_long.csv"), header = TRUE, stringsAsFactors = FALSE)
qualtricsData_raw_long <- qualtricsData_raw_long %>% mutate(tagid=sprintf("TAG%03s",SID)) %>% select(tagid, item, value, survey_name) %>% filter(!tagid=="TAG000")
