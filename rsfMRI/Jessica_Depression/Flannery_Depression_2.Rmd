...
packages <-  c("lme4", "nlme", "ggplot2", "zoo", "dplyr", "tidyr", "knitr",
              "parallel", "data.table", "lubridate", "data.table", "stringr", "summarytools", "purrr")
...
# remove <- KSADS[,(3:272)]
KSADS_reduced <-KSADS[,c(1,2,273:338, 1111:1368)] 
...
KSADS_reduced$ID<- str_replace_all(KSADS_reduced$ID, "_", "")
KSADS_reduced$ID<- str_replace_all(KSADS_reduced$ID, "-", "")
KSADS_wave2 <- KSADS_reduced %>% mutate(sub=paste0("sub-",ID)) %>%
arrange(sub)

CTQ<-fread(paste0("/Volumes/psych-cog/dsnlab/TAG/bids_data/derivatives/Jessica_Depression/CTQ_Wave12.csv")) %>% 
mutate(sub=paste0("sub-",tagid)) %>%
arrange(sub)
  CTQ_reduced <-CTQ[,-c(1,3:71)]  %>%
  filter(grepl("W2", survey_name))
  
KSADS_CESDC_CTQ <- list(CESDC_wave2, KSADS_wave2, CTQ_reduced) %>%
  reduce(full_join, by= "sub") %>%
  arrange(sub)

#because some were named with space in title!
cnames <- names(KSADS_CESDC_CTQ)<-gsub("\\s","_",names(KSADS_CESDC_CTQ))
colnames(KSADS_CESDC_CTQ) <- cnames

KSADS_CESDC_CTQ_gut <- KSADS_CESDC_CTQ %>%
  filter(Participated_GUT == 1)

view(dfSummary(KSADS_CESDC_CTQ_gut[,c(29,30,35:36, 39:40,103:114)]))

###for running locally (no connection to cas)###
