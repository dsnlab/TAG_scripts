...
Dep_Anx <-merge(CESDC,SCARED, by= "sub") 

#get demographics
Parent_SES_1<-fread(paste0("/Volumes/psych-cog/dsnlab/TAG/bids_data/derivatives/Jessica_Depression/FLUX2018/TAG_W1_SES_ParentQ.csv"))
Parent_clean_1 <- Parent_SES_1[ -c(1:3), c(-2, -4, -6)] 
class(Parent_clean_1)
Parent_clean_1<- as.data.frame(Parent_clean_1)

Parent_SES_2<-fread(paste0("/Volumes/psych-cog/dsnlab/TAG/bids_data/derivatives/Jessica_Depression/FLUX2018/TAG_W1_SES_ParentQ_2.csv"))
Parent_clean_2 <- Parent_SES_2[ c(-1, -3), c(-3, -5)] 
class(Parent_clean_2)
Parent_clean_2 <- as.data.frame(Parent_clean_2)
colnames(Parent_clean_2) <- Parent_clean_2[1,]
Parent_clean_2 <- Parent_clean_2[c(-1, -57),]

#make sure variable names are the same
colnames(Parent_clean_1) <- colnames(Parent_clean_2)

#combine datasets
Parent_clean<- rbind(Parent_clean_1, Parent_clean_2)
