...
packages <-  c("lme4", "nlme", "ggplot2", "zoo", "dplyr", "tidyr", "knitr",
              "parallel", "data.table", "lubridate", "plyr")
...
sub_base_dir="/Volumes/psych-cog/dsnlab/TAG/TAG_BIDS/derivatives/rsfMRI_preproc_noFDscrub/"
...
system.time(jess_cois <- mclapply(as.list(sublist$subid),
...
if(rerun_models){
  system.time(
    gordon_models<-mclapply(
      as.list(gordon_cois$coi)[1:50], 
  ...
  rm(gordon_models);gc()
  saveRDS(gordon_modelsDT, paste0(workdir,"gordon_modelsDT.RDS"))
} else {
  gordon_modelsDT <- readRDS(paste0(workdir,"gordon_modelsDT.RDS"))
}
```