...
loessPlot<-ggplot(behavioraldata,
   ...
   scale_colour_manual(values=c('maroon1', 'maroon4'),
                      labels=c('Ordinary', 'Intimate'))+
...
modelPlot<-ggplot(predictedData,
	...
	geom_line(aes(group=tagid), color='hotpink', alpha=.2)+
	geom_line(data=(predictedData %>% filter(affective==TRUE)),
	                aes(y=disclosure_uncond), color='maroon4', size=2)+
  	geom_line(data=(predictedData %>% filter(affective==FALSE)),
	                aes(y=disclosure_uncond), color='maroon1', size=2)+
...
```{r}
run_predict_disclose=TRUE
run_disclose_affiliation=TRUE
...
## Does these connections interact with affective value in predicting disclosure?
if(run_predict_disclose){
  ...
  disclose_modelsDT <- rbindlist(disclose_models) %>%
    mutate(chisq=as.numeric(chisq),
           pval=as.numeric(pval))#make this into a dataframe 
...
mclapply(X=as.character(as.list((disclose_modelsDT %>% filter(mod_type=="coi int model"))$coiname)), 
         demodata=behavioraldata,
         coidat=social_coisDT,
         mc.cores=numcores,
         FUN=function(coi_name, demodata, coidat){
