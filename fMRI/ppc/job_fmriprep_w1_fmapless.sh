...
derivatives="${group_dir}""${study}"/fmriprep_1.5.2/wave1
working_dir="${derivatives}"/working_wave1/
...
singularity run --cleanenv --bind "${group_dir}":"${group_dir}" $image $bids_dir $derivatives \
	...
	--use-syn-sdc --fs-license-file ${group_dir}BIDS/license.txt --ignore slicetiming 
