...
for map in ${maps[@]}; do 
3dAllineate -source "${map_dir}"/"${model}"/"${map}".nii -master "${con_dir}"/mask.nii -final NN -1Dparam_apply '1D: 12@0'\' -prefix "${map_dir}"/"${model}"/aligned_"${map}"
for con in ${cons_files[@]}; do 
echo "${SUB}" "${con}" "${model}" "${map}" `3ddot -dodot "${map_dir}"/"${model}"/aligned_"${map}"+tlrc "${con_dir}"/"${con}"` >> "${output_dir}"/"${SUB}"_dotProducts.txt
done
done
done
...
for con in ${cons_files[@]}; do 
echo "${SUB}" "${con}" "${model}" "${map}" `3ddot -dodot "${map_dir}"/aligned_"${map}"_"${SUB}"+tlrc "${con_dir}"/"${con}"` >> "${output_dir}"/"${SUB}"_dotProducts.txt
done
