...
# ------------------------------------------------------------------------------------------
# paths
fx_dir='/projects/dsnlab/shared/tag/nonbids_data/fMRI/fx/models/dsd/wave1/pmod/MLmotion_AR_RT' #FX input directory
...
fx_cons=(con_0001 con_0002 con_0003 con_0004 con_0005 con_0006 con_0007 con_0008 con_0009 con_0010 con_0011 con_0012 con_0013 con_0014 con_0015) #fx con files to extract from
rois=(lNAcc_25 rNAcc_25 vmPFC)

for roi in ${rois[@]}; do 
	for sub in ${subjects[@]}; do 
		for con in ${fx_cons[@]}; do 
			if [ -a $fx_dir/$sub/${con}.nii ]; then
				echo ${sub} ${con} `3dmaskave -sigma -quiet -mrange 1 1 -mask $output_dir/masks/${roi}_resample.nii.gz $fx_dir/$sub/${con}.nii` >> $output_dir/paramEstimates_${roi}_AR.txt
			fi
		done
	done
done
