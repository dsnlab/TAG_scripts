...
for roi in ${rois[@]}; do 
	for wave in ${waves[@]}; do 
		for contrast in ${contrasts[@]}; do 
			echo "${SUB}" "${wave}" "${roi}" "${contrast}" `3dmaskave -sigma -quiet -mask "${roi_dir}"/"${roi}"_cluster+tlrc "${contrast_dir}"/"${wave}"/con_"${contrast}".nii` >> "${output_dir}"/"${roi}"_SvC_ParameterEstimates.txt
		done
	done
done
