...
echo "ROI mean sigma" >> "${output_dir}"/meanICC_block.txt
echo "${roi}" `3dmaskave -sigma -quiet -mask "${roi_dir}"/aligned_"${roi}"+tlrc "${output_dir}"/ICC_block+tlrc[Subj]` >> "${output_dir}"/meanICC_block.txt
echo "min" `3dmaskave -min -quiet -mask "${roi_dir}"/aligned_"${roi}"+tlrc "${output_dir}"/ICC_block+tlrc[Subj]` >> "${output_dir}"/meanICC_block.txt
echo "max" `3dmaskave -max -quiet -mask "${roi_dir}"/aligned_"${roi}"+tlrc "${output_dir}"/ICC_block+tlrc[Subj]` >> "${output_dir}"/meanICC_block.txt
