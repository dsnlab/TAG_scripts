...
df <- df %>% filter(sid == id & wave == curr_wave)
...
if(nrow(df) > 0){
  ...
  df_disc_nod <- df %>% 
    mutate(name = paste("trial", df$trial, sep = "_"),
           onset = disc.onset,
           duration = ifelse(is.na(disc.rt.seconds), 2, disc.rt.seconds)) %>% 
  ...
  df_eval_nod <- df %>% 
    mutate(name = paste("trial", df$trial, sep = "_"),
           ...
           duration = ifelse(is.na(statement.rt.seconds), 2, statement.rt.seconds)) %>% 
    select(subject_id, run, name, onset, duration, eval_missed, intimate_cond, disclosed, share_value_cond) 
  
  df_eval_nod$run <- as.factor(df_disc_nod$run)
  
  #save disclosure summaries
  saveDisc <- df_disc_nod %>% group_by(run) %>%
    do({
      ungroup(.)
      write.csv(.,  file=paste0(out_dir, 'disclosure_nod/wave', curr_wave, '/TAG',fid, 'w0', curr_wave, '_DSD', .$run[[1]],'_disc_betaseries_summary.csv'),row.names = F,na = "",)
      data.frame()
      })
  ...
  saveEval <- df_eval_nod %>% group_by(run) %>%
    do({
      ungroup(.)
      write.csv(., file = paste0(out_dir, 'eval_nod/wave', curr_wave, '/TAG',fid, 'w0', curr_wave, '_DSD', .$run[[1]],'_eval_betaseries_summary.csv'),row.names = F,na = "",)
      data.frame()
      })
