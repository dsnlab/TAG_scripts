...
#Of the final list, create file outlining multiconds in run 1 and run 2 for subjects, and whether they differ between runs
```{r}
Con_bySubj <- summary_bySubj %>% filter(sid %in% final_list$SID) %>% spread(key=run,value=multicond) %>%
  rename(run1 = `1`,
         run2 = `2`) %>%
  mutate(same = ifelse(run1 == run2, 1, 0))
write.csv(allCon_bySubj, '~/Desktop/Con_bySubj.csv',row.names=F)
```
