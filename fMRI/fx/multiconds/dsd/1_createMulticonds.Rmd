...
#Save subject-specific files for behavioral analyses in Matlab
```{r}
files <- lapply(X=ids, df=out_df, FUN=function(id, df) {
  ...
  write.table(df,file=paste0(file_dir,'tag',fid,'.csv'),sep=",",row.names=F,col.names=F,na = "") 
   })
...
summary_acrossRun_byShareValue <- multiconds %>% 
  mutate(share.value = ifelse(pmod < 0, -1,
                       ifelse(pmod > 0, 1,
                              ifelse(pmod == 0, 0, NA)))) %>%
  mutate(name=as.factor(name)) %>%
  group_by(sid,run,name,share.value) %>% summarize(N = n()) %>% 
  filter(name %in% c(4,6)) %>% 
  group_by(name,share.value) %>%
  summarize(N = 100*(n()/312))
...
summary_acrossRun_byShareValue_over3 <- multiconds %>% 
  mutate(share.value = ifelse(pmod < 0, -1,
                       ifelse(pmod > 0, 1,
                              ifelse(pmod == 0, 0, NA)))) %>%
  mutate(name=as.factor(name)) %>%
  group_by(sid,name,share.value) %>% summarize(N = n()) %>% 
  filter(name %in% c(4,6)) %>% 
  filter(N > 3) %>%
  group_by(name,share.value) %>%
  summarize(N = 100*(n()/156))
...
write.csv(dsd_overview,'~/Desktop/dsd_overview.csv')

compIDS <- dsd_overview %>% filter(is.na(technical_missing) & is.na(participant_error) & dsd_completed == 2) %>% select(SID)
