...
check_ids <- rbindlist(multiconds, fill=T)
multiconds <- check_ids %>% select(sid,run,name,onset,duration,pmod) %>% filter(!is.na(name))
check_ids <- check_ids %>% select(sid,run1,run2) %>% filter(!is.na(run1))
...
multiconds2 <- multiconds %>% filter(!name==1) %>% filter(!name==2) %>% 
  group_by(sid,run,name) %>%
  summarise(N=n()) %>%
...
levels(multiconds2$name) <- c("Neutral_Private", "Neutral_Share"
                              ,"Affect_Private", "Affect_Share")
levels(multiconds2$run) <- c("Run1","Run2")
