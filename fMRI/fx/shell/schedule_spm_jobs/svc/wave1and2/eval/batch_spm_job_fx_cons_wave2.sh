...
SCRIPT=${STUDY}/fMRI/fx/models/svc/wave1and2/fx_eval_cons_wave2.m

# Set shell script to execute
SHELL_SCRIPT=spm_job_residuals_wave2.sh

# Tag the results files
RESULTS_INFIX=fx_eval_cons
...
for SUB in $SUBJLIST; do
		sbatch --export ALL,REPLACESID=$REPLACESID,SCRIPT=$SCRIPT,SUB=$SUB,SPM_PATH=$SPM_PATH  \
			--job-name=${RESULTS_INFIX} \
		 	-o ${OUTPUTDIR}/${SUB}_wave2_${RESULTS_INFIX}.log \
