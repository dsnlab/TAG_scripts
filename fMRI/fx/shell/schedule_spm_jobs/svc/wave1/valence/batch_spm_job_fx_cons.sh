...
SCRIPT=${STUDY}/fMRI/fx/models/svc/wave1/fx_valence_cons.m

# Set shell script to execute
SHELL_SCRIPT=spm_job_residuals.sh

# Tag the results files
RESULTS_INFIX=fx_valence_cons

# Set output dir and make it if it doesn't exist
OUTPUTDIR=${STUDY}/fMRI/fx/shell/schedule_spm_jobs/svc/wave1/valence/output
...
for SUB in $SUBJLIST; do
		sbatch --export ALL,REPLACESID=$REPLACESID,SCRIPT=$SCRIPT,SUB=$SUB,SPM_PATH=$SPM_PATH  \
			--job-name=${SUB}_${RESULTS_INFIX} \
		 	-o ${OUTPUTDIR}/${SUB}_${RESULTS_INFIX}.log \
		 	--cpus-per-task=${cpuspertask} \
		 	--mem-per-cpu=${mempercpu}  --account=dsnlab --partition=ctn\
		 	${SHELL_SCRIPT}
			sleep .25
done
