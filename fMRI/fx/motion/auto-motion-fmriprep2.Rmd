...
This script will create an artifact regressor from fmriprep confounds, and combine them with the euclidean regressors to create a final motion regressor file. 
...
motionDir = '/Volumes/psych-cog/dsnlab/TAG/nonbids_data/fMRI/fx/motion/wave1/auto-motion-fmriprep/rp_txt/fd0.8_dv2.0_Neighbors/'
...
if (!file.exists(paste0(scriptDir,'/fmriprep_motion.csv'))) {
...
```{r}
if (!file.exists(paste0(scriptDir,'/euclidean_motion.csv'))) {
  ...
  write.csv(eucDF, paste0(scriptDir,'euclidean_motion.csv'), row.names = FALSE)  
  
} else {
  eucDF = read.csv(paste0(scriptDir,'euclidean_motion.csv'), stringsAsFactors = FALSE)
}
  ...
  group_by(SID,task) %>%
    do({
      plot = ggplot(., aes(volume, value)) + 
        ...
        labs(title = .$SID[[1]])
      print(plot)
      ggsave(plot, file = paste0(plotDir,.$SID[[1]],'_',.$task[[1]],'.pdf'), height = 10, width = 12)
      data.frame()
    })
```