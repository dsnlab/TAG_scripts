...
```{r, include = F}
MRIoverview <- read.csv(paste0('behavior/MRI/Wave1/MRI_W',wave,'overview.csv'),header=T)
...
motion <- lapply(X=motionFiles, FUN=function(file) {
  ...
  #write.table(motionData, file=paste0(cas_dir,'nonbids_data/fmri/fx/motion/wave',wave,'/TAG',SID,'_',task,run,'.txt'), sep='\t', row.name=F, quote=F, col.names=F)
