  ...
  for (file in fileList) {
    tmp = tryCatch(read_tsv(file.path(confoundDir, file)) %>% 
                     ...
                     mutate(wave = str_extract(wave, "[[:digit:]]+"),
                            ...
                            volume = row_number()) %>%
                     dplyr::select(subjectID, wave, task, run, volume, everything()), error = function(e) message(file))
    # add missing columns and select subset classifier columns
    missingColumns = setdiff(columnNames, names(tmp))
    tmp[missingColumns] = 0 
    
    tmp = tmp  %>%
      dplyr::select(subjectID, wave, task, run, volume, CSF, WhiteMatter, 
             GlobalSignal, stdDVARS, non.stdDVARS, vx.wisestdDVARS, 
             FramewiseDisplacement, tCompCor00, tCompCor01, tCompCor02, 
             tCompCor03, tCompCor04, tCompCor05, aCompCor00, aCompCor01, 
             aCompCor02, aCompCor03, aCompCor04, aCompCor05, Cosine00, 
...
summaryTrash = dataset %>%
  filter(trash == 1) %>%
  dplyr::select(subjectID, wave, task, run, volume, trash)

# fd by task
summaryTaskFD = dataset %>% 
  group_by(subjectID, wave, task) %>% 
  summarise(maxFD = max(FramewiseDisplacement, na.rm = T))
...
write.csv(summaryTaskFD, file.path(summaryDir, paste0(study, '_summaryTaskFD.csv')), row.names = FALSE)
...
rps = dataset %>%
  dplyr::select(subjectID, wave, task, run, volume, X, Y, Z, RotX, RotY, RotZ, trash)
...
if (noRP == FALSE) {
  ...
  if (noEuclidean == FALSE) {
    ...
    rps = rps %>%
      ...
      dplyr::select(subjectID, wave, task, run, volume, trans, rot, deriv.trans, deriv.rot, trash)
