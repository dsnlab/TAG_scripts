...
set working_dir = /projects/dsnlab/shared/tag/nonbids_data/fMRI/fx/models/dsd/wave1/afni/main
...
afni_proc.py -subj_id $subj                                      \
	...
	-regress_local_times					 \
        -regress_stim_times                                      \
            $stim_dir/${subj}_affSTATE.txt                            \
            $stim_dir/${subj}_neutSTATE.txt                           \
            $stim_dir/${subj}_affSH.txt                               \
            $stim_dir/${subj}_neutSH.txt                              \
            $stim_dir/${subj}_affPRI.txt                              \
            $stim_dir/${subj}_neutPRI.txt                             \
	    $stim_dir/${subj}_missing.txt			      \
	-regress_stim_types times times times times times times times \
        -regress_stim_labels                                   	      \
            affSTATE neutSTATE affSH neutSH affPRI neutPRI missing    \
        -regress_basis_multi 'BLOCK(4,1)' 'BLOCK(4,1)' 'BLOCK(3,1)' 'BLOCK(3,1)' 'BLOCK(3,1)' 'BLOCK(3,1)' 'BLOCK(3,1)' \
