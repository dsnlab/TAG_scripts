...
adf <- df %>% filter(sid==id) %>% ungroup() %>% 
  mutate(affect_dec_pmod = ifelse(cond3_remove==1,0,1),
         neutral_dec_pmod = ifelse(cond4_remove==1,0,1))
run1 <- adf %>% filter(run==1) %>% select(-sid,-run) %>% mutate(affect_state=1, neutral_state=1, affect_decision=1, affect_value_pmod=1,neutral_decision=1, neutral_value_pmod=1) %>%
  select(affect_state,neutral_state,affect_decision,affect_value_pmod,affect_dec_pmod,neutral_decision,neutral_value_pmod,neutral_dec_pmod)
...
contrasts <- c("affect_share","neutral_share","affect_decision","neutral_decision","affect_value_pmod","neutral_value_pmod","affect_dec_pmod","neutral_dec_pmod")
