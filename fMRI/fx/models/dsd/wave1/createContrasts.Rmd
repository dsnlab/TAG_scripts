...
df <- df %>% 
  mutate(decision_aff_neut = affect_decision-neutral_decision,
         ...
         affect_dec_state = affect_decision - state,
         neutral_dec_state = neutral_decision - state)
...
df <- df %>% select(state,affect_state,neutral_state,affect_decision,neutral_decision,decision_aff_neut,decision_neut_aff,affect_value_pmod,neutral_value_pmod,valueP_aff_neut,valueP_neut_aff,affect_dec_state,neutral_dec_state,everything())
...
%-----------------------------------------------------------------------")
cat("\n")
cat("matlabbatch{1}.spm.stats.con.spmmat = {'/Volumes/psych-cog/dsnlab/TAG/nonbids_data/fMRI/fx/models/dsd/wave1/pmod/MLmotion_FAST/sub-TAG",fid,"/SPM.mat'};",sep="")
cat("\n")
for(i in 1:length(df)){
cat("matlabbatch{1}.spm.stats.con.consess{",i,"}.tcon.name = '", colnames(df)[i],"';", sep="")
cat("\n")
