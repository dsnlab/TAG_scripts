...
out_dir <- '/Volumes/StorEDGE/WORK/DSN/TAG/TAG_scripts/fMRI/fx/models/dsd/wave1/contrasts/pmod/MLmotion/'
```
...
df <- df %>% 
  mutate(decision_aff_neut = affect_decision-neutral_decision,
...
df <- df %>% select(state,affect_state,neutral_state,affect_decision,neutral_decision,decision_aff_neut,decision_neut_aff,affect_value_pmod,neutral_value_pmod,valueP_aff_neut,valueP_neut_aff,everything())
...
%-----------------------------------------------------------------------")
cat("\n")
cat("matlabbatch{1}.spm.stats.con.spmmat = {'/Volumes/psych-cog/dsnlab/TAG/nonbids_data/fMRI/fx/models/dsd/wave1/MLmotion/pmod/sub-TAG",fid,"/SPM.mat'};",sep="")
cat("\n")
for(i in 1:length(df)){
cat("matlabbatch{1}.spm.stats.con.consess{",i,"}.tcon.name = '", colnames(df)[i],"';", sep="")
cat("\n")
...
#both dec_pmod
dec_pmod <- conn_summaryDF %>% filter(contrast %in% c("affect_dec_pmod","neutral_dec_pmod"))
pmod_list <- dec_pmod %>% group_by(id) %>% summarise(N=n()) %>% filter(N==2) %>% select(id)

###create pmod subject list
write.csv(pmod_list,'~/Desktop/subject_list_pmod.txt',row.names=F)