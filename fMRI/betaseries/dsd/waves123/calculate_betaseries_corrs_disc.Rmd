---
title: "calculate_betaseries_corrs_disc"
...
# Trial type
trial_type_abbrev <- "disc_dec" #full_trial
trial_type_full <- "disclosure"

# Inputs
param_input_dir <- paste0("/Volumes/psych-cog/dsnlab/cheng_spf/data/betaseries_paramEst/")
task_summary_dir <- paste0("/Volumes/psych-cog/dsnlab/TAG/nonbids_data/fMRI/fx/multiconds/dsd/betaseries/", trial_type_full, "_summary/")
subjects_dir <- "/Volumes/psych-cog/dsnlab/cheng_spf/subjects/dsd_subject_lists"
fx_roi_dir <- "disc_dec" #/vmPFC_symm_parameterEstimates

# Outputs 
output_dir <- "/Volumes/psych-cog/dsnlab/cheng_spf/data/betaseries_paramEst/summaries/disc_dec"
```
...
merged_full_trial_summary_files <- read.csv(paste0(task_summary_dir, "merged_", trial_type_full, "_summary_files.csv"))
...
merged_full_trial_summary_files <- filter(merged_full_trial_summary_files, !(subject_id == "094w02" & is.na(disc.rt.seconds))) %>% 
  filter(!(subject_id == "094w02" & is.na(statement.rt.seconds))) # manully removed bad trials

# run 1 was duplicated, use second rep for 030w01
merged_full_trial_summary_files <- filter(merged_full_trial_summary_files, !(subject_id == "030w01" & X %in% (2212:2252)))
...
create_betas <- apply(file_list_single_run_df, 1, function(x) { # file_list_both_runs_df, file_list_single_run_df, file_list_atypical_runs_df
  # x = file_list_atypical_runs_df[3, ] # for testing
  #x= file_list_both_runs_df[43, ]
  #x= file_list_single_run_df[1, ]
  ...
  df_NAcc <- read.delim(paste0(param_input_dir, trial_type_abbrev, "/", x[[2]]), sep = "", header = FALSE)
  df <- rbind(df, df_NAcc)
  ...
  if (!(length(df) == 5)) {
    print("df doesn't have 5 columns")
      ...
      if (how_many_runs == "single_run") {
       ...
       if (how_many_runs == "atypical_runs") {
        df_wide <- df %>% 
          filter(!is.na(mean)) %>% 
          pivot_wider(names_from = roi, values_from = c(mean, sd)) %>% 
          filter(beta != "beta_0088.nii")
        trial_data <- filter(merged_full_trial_summary_files, subject_id == x[[1]])
        ...
        #df_wide <- df_wide[42:82, ] # 41:82
        #trial_data <- trial_data[42:82, ] # trial_data[1:82, ]
      ...
      roilist <- c("inflated_vmPFC", "primary_aud_sphere_rad4_n56_n16_0", "primary_vis_sphere_rad4_n4_n88_4")
...
#df_betas_both_runs <- do.call(rbind, create_betas)
df_betas_single_run <- do.call(rbind, create_betas)
# df_betas_005w01 <- do.call(rbind, create_betas)
# df_betas_054w01 <- do.call(rbind, create_betas)

all_but_210w01 <- rbind(df_betas_both_runs, df_betas_single_run, df_betas_005w01, df_betas_054w01)
saveRDS(all_but_210w01, paste0(output_dir, "/orig_rois/all_but210w01.rds"))
## STOPPED HERE, NEED TO RUN 3 PARTICIPANTS W ATYPICAL RUNS THEN RBIND EM ALL TOGETHER
