...
overview <- overview %>% full_join(.,freesurfer) %>% full_join(.,fmriprep) %>% 
  ...
  mutate_at(.vars = c("mprage_num","mprage_size","svc1_num","svc1_size","svc2_num","svc2_size","rest1_num","rest1_size","rest2_num","rest2_size","dsd1_num","dsd1_size","dsd2_num","dsd2_size","fmap_num","fmap_size","diffRL_num","diffRL_size","diffLR_num","diffLR_size"), funs(ifelse(check_mri_completed == 0, NA, .))) #If mri was not completed at wave 1, change all MRI fields are NA (sometimes not NA if subject skipped wave 1, but did wave 2 - because wave 2 would be coded as wave 1 on bids_data in these situations). 
