...
master_check2 <- filter(master, grepl('<', DHEA) | grepl('<', Estradiol) | grepl('<', Testosterone)))
master_check <- rbind(master_check1,master_check2)
...
Saliva <- master_check %>% 
  ...
  left_join(.,decisions,by=c("SID","Sample")) %>%
  ...
  select(SID,Sample,sal_DHEA,sal_TEST,sal_EST) %>%
  filter(!is.na(SID)) %>%
...
#import and format med "full" dataset ##NEED TO CREATE THIS FILE!!
control_meds <- read.csv('~/Desktop/TAG_W2_Saliva_meds_processed_full.csv')
...
control_days <- read.csv(paste0(saliva_dir,'/TAG_W2_Saliva_dates.csv'))
...
puberty <- read.csv(paste0(puberty_dir,'/TAG_W2_PubertyComposite.csv'))
puberty <- puberty %>% select(SID,PUBcomp) %>% mutate(SID = str_pad(SID, 3, pad="0"))

age <- read.xlsx(paste0(age_dir,'/TAG_Age.xlsx'), sheetIndex=1)
age <- age %>% mutate(SID=str_replace(TAG_ID,"TAG","")) %>% 
  filter(!is.na(SID)) %>%
  filter(wave==2 & session==2) %>%
  rename(ageS2=age_excel)
  select(SID,ageS2)
...
```{r}
#save raw saliva concentrations (clean up variable names first)
Saliva <- Saliva %>% rename(completed_form = complete,
                            form_date = doc,
                            medcode_1 = code_1,
                            medcode_13 = code_13,
                            medcode_16 = code_16,
...
#save basal concentrations
Saliva_basal <- Saliva_basal %>% select(-grp) 
write.csv(Saliva_basal,paste0(saliva_dir,'/TAG_W2_Saliva_basalestimates.csv'),row.names=F)
