...
Sal_DHEA_final <- read_xlsx(paste0(saliva_dir,'/TAG_SALIVA_Master_File.xlsx'), sheet=4)
Sal_TEST_final <- read_xlsx(paste0(saliva_dir,'/TAG_SALIVA_Master_File.xlsx'), sheet=8)
Sal_EST_final <- read_xlsx(paste0(saliva_dir,'/TAG_SALIVA_Master_File.xlsx'), sheet=12)

#check entries with '?????' listed in concentration columns -
#these refer to either entries that are too high or too low.
DHEA_check <- Sal_DHEA_final %>% filter(Concentration =='?????') 
TEST_check <- Sal_TEST_final %>% filter(Concentration =='?????') 
EST_check <- Sal_EST_final %>% filter(Concentration =='?????') 

###NOTE: NV & MB made decisions about how to update/use each of these '?????' values.
...
###DISTRIBUTIONS OF CONCENTRATIONS & LOG TRANSFORMATIONS
...
Saliva_long <- Saliva_long %>% 
  mutate(concentration_orig = concentration,
         concentration = log(concentration + 4)) ###NOTE FROM HERE ON, "CONCENTRATION" IS LOG TRANSFORMED###
