...
final_il12_summary
final_il2_summary <- describe(tag_w1_immune$il2_raw) 
final_il2_summary
...
```
In this data, there is only 1 left outlier for IFN-g and 1 left outlier for IL-10. Winsorize those.
```{r}
library(DescTools)
tag_w1_immune$ifng_w <- Winsorize(tag_w1_immune$ln_ifng, minval = ifng_lo_limit, maxval = ifng_up_limit, na.rm = TRUE)
tag_w1_immune$il10_w <- Winsorize(tag_w1_immune$ln_il10, minval = il10_lo_limit, maxval = il10_up_limit, na.rm = TRUE)
```
...
tag_w1_immune$il10_cleaned <- tag_w1_immune$il10_w
