...
library(readr)
knitr::opts_knit$set(root.dir = 'I:/dsnlab/TAG/behavior/Immune/Wave1')
```
...
min_ifng <- min(na.omit(ifng_num$ifng_raw))
...
min_il10 <- min(na.omit(il10_num$il10_raw))
...
min_il12 <- min(na.omit(il12_num$il12_raw))
...
min_il2 <- min(na.omit(il2_num$il2_raw))
...
min_il4 <- min(na.omit(il4_num$il4_raw))
...
min_tnfa <- min(na.omit(tnfa_num$tnfa_raw))
...
min_iga <- min(na.omit(iga_num$iga_raw))
...
# Note: may want to consider imputing missing durations for siga, otherwise they are NA
```
...
#Note: although it is common to do a natural log or square root transformation, these transfromations did not result in acceptable skew/kurtosis stats. Therefore, we chose to use the transform Tukey function, which uses iterative Shapiro-Wilk tests, and finds the lambda values that maximizes the W statistic from those tests. (e.g., find power transformation that makes the data fit the normal distribution as closely as possible). See info on rcompanion for more.
library(rcompanion)
if (crp_summary$skew > 2 | crp_summary$skew < -2 | crp_summary$kurtosis > 2 | crp_summary$kurtosis < -2) {
  tag_w1_immune$ln_crp <- transformTukey(tag_w1_immune$crp_raw)
}
if (ifng_summary$skew > 2 | ifng_summary$skew < -2 | ifng_summary$kurtosis > 2 | ifng_summary$kurtosis < -2) {
  tag_w1_immune$ln_ifng <- transformTukey(tag_w1_immune$ifng_raw)
}
if (il10_summary$skew > 2 | il10_summary$skew < -2 | il10_summary$kurtosis > 2 | il10_summary$kurtosis < -2) {
  tag_w1_immune$ln_il10 <- transformTukey(tag_w1_immune$il10_raw)
}
if (il12_summary$skew > 2 | il12_summary$skew < -2 | il12_summary$kurtosis > 2 | il12_summary$kurtosis < -2) {
  tag_w1_immune$ln_il12 <- transformTukey(tag_w1_immune$il12_raw)
}
if (il2_summary$skew > 2 | il2_summary$skew < -2 | il2_summary$kurtosis > 2 | il2_summary$kurtosis < -2) {
  tag_w1_immune$ln_il2 <- transformTukey(tag_w1_immune$il2_raw)
}
if (il4_summary$skew > 2 | il4_summary$skew < -2 | il4_summary$kurtosis > 2 | il4_summary$kurtosis < -2) {
  tag_w1_immune$ln_il4 <- transformTukey(tag_w1_immune$il4_raw)
}
if (il6_summary$skew > 2 | il6_summary$skew < -2 | il6_summary$kurtosis > 2 | il6_summary$kurtosis < -2) {
  tag_w1_immune$ln_il6 <- transformTukey(tag_w1_immune$il6_raw)
}
if (tnfa_summary$skew > 2 | tnfa_summary$skew < -2 | tnfa_summary$kurtosis > 2 | tnfa_summary$kurtosis < -2) {
  tag_w1_immune$ln_tnfa <- transformTukey(tag_w1_immune$tnfa_raw)
}
if (siga_summary$skew > 2 | siga_summary$skew < -2 | siga_summary$kurtosis > 2 | siga_summary$kurtosis < -2) {
  tag_w1_immune$ln_siga <- transformTukey(tag_w1_immune$siga_frc)
}
# Check everything post transformation for any transformed variables 
final_crp_summary <- describe(tag_w1_immune$ln_crp)
final_crp_summary
final_ifng_summary <- describe(tag_w1_immune$ln_ifng)
final_ifng_summary
final_il10_summary <- describe(tag_w1_immune$ln_il10)
final_il10_summary
final_il12_summary <- describe(tag_w1_immune$ln_il12)
final_il12_summary
final_il2_summary <- describe(tag_w1_immune$ln_il2) 
final_il2_summary
final_il4_summary <- describe(tag_w1_immune$ln_il4) 
final_il4_summary
final_il6_summary <- describe(tag_w1_immune$ln_il6)
final_il6_summary 
final_tnfa_summary <- describe(tag_w1_immune$ln_tnfa)
final_tnfa_summary
final_siga_summary <- describe(tag_w1_immune$ln_siga)
final_siga_summary
hist(tag_w1_immune$ln_crp)
hist(tag_w1_immune$ln_ifng)
hist(tag_w1_immune$ln_il10)
hist(tag_w1_immune$ln_il12)
hist(tag_w1_immune$ln_il2)
...
ggplot(tag_w1_immune, aes(x = "", y = ln_crp)) +   
  geom_boxplot() +
  ylab("transformed (CRP)") +
  ggtitle("TAG W1 CRP (transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(tag_w1_immune, aes(x = "", y = ln_ifng)) +   
  geom_boxplot() +
  ylab("transformed (IFN-g)") +
  ggtitle("TAG W1 IFN-g (transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(tag_w1_immune, aes(x = "", y = ln_il10)) +   
  geom_boxplot() +
  ylab("transformed (IL-10)") +
  ggtitle("TAG W1 IL-10 (transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(tag_w1_immune, aes(x = "", y = ln_il12)) +   
  geom_boxplot() +
  ylab("transformed (IL-12)") +
  ggtitle("TAG W1 IL-12 (transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(tag_w1_immune, aes(x = "", y = ln_il2)) +   
  geom_boxplot() +
  ylab("transformed (IL-2)") +
  ggtitle("TAG W1 IL-2 (transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(tag_w1_immune, aes(x = "", y = ln_il4)) +   
  geom_boxplot() +
  ylab("transformed (IL-4)") +
  ggtitle("TAG W1 IL-4 (transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(tag_w1_immune, aes(x = "", y = ln_il6)) +   
  geom_boxplot() +
  ylab("transformed (IL-6)") +
  ggtitle("TAG W1 IL-6 (transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(tag_w1_immune, aes(x = "", y = ln_tnfa)) +   
  geom_boxplot() +
  ylab("transformed (TNF-a)") +
  ggtitle("TAG W1 TNF-a (transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(tag_w1_immune, aes(x = "", y = ln_siga)) +   
  geom_boxplot() +
  ylab("transformed (SIgA)") +
  ggtitle("TAG W1 SIgA (transformed)") +
  geom_smooth(method='lm', color="black")
```
...
crp_lo_limit
ifng_up_limit <- (final_ifng_summary$mean + 3*(final_ifng_summary$sd))
ifng_up_limit
ifng_lo_limit <- (final_ifng_summary$mean - 3*(final_ifng_summary$sd))
ifng_lo_limit
il10_up_limit <- (final_il10_summary$mean + 3*(final_il10_summary$sd))
il10_up_limit
il10_lo_limit <- (final_il10_summary$mean - 3*(final_il10_summary$sd))
il10_lo_limit
il12_up_limit <- (final_il12_summary$mean + 3*(final_il12_summary$sd))
il12_up_limit
il12_lo_limit <- (final_il12_summary$mean - 3*(final_il12_summary$sd))
il12_lo_limit
il2_up_limit <- (final_il2_summary$mean + 3*(final_il2_summary$sd))
il2_up_limit
il2_lo_limit <- (final_il2_summary$mean - 3*(final_il2_summary$sd))
il2_lo_limit
il4_up_limit <- (final_il4_summary$mean + 3*(final_il4_summary$sd))
il4_up_limit
il4_lo_limit <- (final_il4_summary$mean - 3*(final_il4_summary$sd))
il4_lo_limit
il6_up_limit <- (final_il6_summary$mean + 3*(final_il6_summary$sd))
il6_up_limit
il6_lo_limit <- (final_il6_summary$mean - 3*(final_il6_summary$sd))
il6_lo_limit
tnfa_up_limit <- (final_tnfa_summary$mean + 3*(final_tnfa_summary$sd))
tnfa_up_limit
tnfa_lo_limit <- (final_tnfa_summary$mean - 3*(final_tnfa_summary$sd))
tnfa_lo_limit
...
crp_num_right_outliers <- length(na.omit(tag_w1_immune$ln_crp [tag_w1_immune$ln_crp > crp_up_limit])) 
crp_num_right_outliers
crp_num_left_outliers <- length(na.omit(tag_w1_immune$ln_crp [tag_w1_immune$ln_crp < crp_lo_limit]))
crp_num_left_outliers
ifng_num_right_outliers <- length(na.omit(tag_w1_immune$ln_ifng [tag_w1_immune$ln_ifng > ifng_up_limit])) 
ifng_num_right_outliers
ifng_num_left_outliers <- length(na.omit(tag_w1_immune$ln_ifng [tag_w1_immune$ln_ifng < ifng_lo_limit])) 
ifng_num_left_outliers
il10_num_right_outliers <- length(na.omit(tag_w1_immune$ln_il10 [tag_w1_immune$ln_il10 > il10_up_limit])) 
il10_num_right_outliers
il10_num_left_outliers <- length(na.omit(tag_w1_immune$ln_il10 [tag_w1_immune$ln_il10 < il10_lo_limit])) 
il10_num_left_outliers
il12_num_right_outliers <- length(na.omit(tag_w1_immune$ln_il12 [tag_w1_immune$ln_il12 > il12_up_limit])) 
il12_num_right_outliers
il12_num_left_outliers <- length(na.omit(tag_w1_immune$ln_il12 [tag_w1_immune$ln_il12 < il12_lo_limit])) 
il12_num_left_outliers
il2_num_right_outliers <- length(na.omit(tag_w1_immune$ln_il2 [tag_w1_immune$ln_il2 > il2_up_limit])) 
il2_num_right_outliers
il2_num_left_outliers <- length(na.omit(tag_w1_immune$ln_il2 [tag_w1_immune$ln_il2 < il2_lo_limit]))
il2_num_left_outliers
il4_num_right_outliers <- length(na.omit(tag_w1_immune$ln_il4 [tag_w1_immune$ln_il4 > il4_up_limit])) 
il4_num_right_outliers
il4_num_left_outliers <- length(na.omit(tag_w1_immune$ln_il4 [tag_w1_immune$ln_il4 < il4_lo_limit]))
il4_num_left_outliers
il6_num_right_outliers <- length(na.omit(tag_w1_immune$ln_il6 [tag_w1_immune$ln_il6 > il6_up_limit])) 
il6_num_right_outliers
il6_num_left_outliers <- length(na.omit(tag_w1_immune$ln_il6 [tag_w1_immune$ln_il6 < il6_lo_limit]))
il6_num_left_outliers
tnfa_num_right_outliers <- length(na.omit(tag_w1_immune$ln_tnfa [tag_w1_immune$ln_tnfa > tnfa_up_limit])) 
tnfa_num_right_outliers
tnfa_num_left_outliers <- length(na.omit(tag_w1_immune$ln_tnfa [tag_w1_immune$ln_tnfa < tnfa_lo_limit])) 
tnfa_num_left_outliers 
siga_num_right_outliers <- length(na.omit(tag_w1_immune$ln_siga [tag_w1_immune$ln_siga > siga_up_limit]))
siga_num_right_outliers 
siga_num_left_outliers <- length(na.omit(tag_w1_immune$ln_siga [tag_w1_immune$ln_siga < siga_lo_limit]))
siga_num_left_outliers
```

In this data, there is only 1 left outlier for IFN-g. Winsorize that.
```{r}
library(DescTools)
tag_w1_immune$ifng_w <- Winsorize(tag_w1_immune$ln_ifng, minval = ifng_lo_limit, maxval = ifng_up_limit, na.rm = TRUE)
```

Output final cleaned data
```{r}
tag_w1_immune$crp_cleaned <- tag_w1_immune$ln_crp
tag_w1_immune$ifng_cleaned <- tag_w1_immune$ifng_w
tag_w1_immune$il10_cleaned <- tag_w1_immune$ln_il10
tag_w1_immune$il12_cleaned <- tag_w1_immune$ln_il12
tag_w1_immune$il2_cleaned <- tag_w1_immune$ln_il2
tag_w1_immune$il4_cleaned <- tag_w1_immune$ln_il4
tag_w1_immune$il6_cleaned <- tag_w1_immune$ln_il6
tag_w1_immune$tnfa_cleaned <- tag_w1_immune$ln_tnfa
tag_w1_immune$siga_cleaned <- tag_w1_immune$ln_siga
