...
BFI10_Wave1_totals_graph<-ggplot(BFI10_Wave1_totals, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+
  ggtitle(paste0("BFI Wave1 total for ",length(unique(BFI10_Wave1_totals$tagid))," participants"))
...
BFI10_Wave1_means_graph<-ggplot(BFI10_Wave1_means, aes(x=value, colour=item)) +
  geom_density(alpha=.3) +
  ggtitle(paste0("BFI Wave1 means for ",length(unique(BFI10_Wave1_means$tagid))," participants"))
...
options(scipen=999)
PDS_Wave1_pdss_by_age_graph<-ggplot((left_join(PDS_Wave1,redcap_cleaned,by="tagid") %>%
                                       filter(!is.na(survey_date),!survey_date=="")%>%
  mutate(age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "years")),2))), aes(x=age,y=pdss)) +
  geom_point(show.legend = FALSE)+
  ggtitle(paste0("PDSS by age for ",length(PDS_Wave1$pdss[!is.na(PDS_Wave1$pdss)])," Wave 1 participants. r = ",
                 round(cor.test(test$age,test$pdss,use=na.or.complete)[[4]],3)," p = ",
                 cor.test(test$age,test$pdss,use=na.or.complete)[[3]]))
...
```{r}
PBIP<-filter(cleaned_survey_data, grepl("PBIP",item)) %>% 
  mutate(value=as.numeric(value)) %>% 
  filter(!is.na(value)) %>% 
  distinct(tagid,item,value,survey_name,.keep_all = FALSE) %>% 
  spread(item,value)
