...
prepareGUIDs=function(subID){
  ...
  assign(guid_colnames[5], month(parse_date_time(sub_NDA_info$dob[1], order=c('mdy'))))
  assign(guid_colnames[6], day(parse_date_time(sub_NDA_info$dob[1], order=c('mdy'))))
  assign(guid_colnames[7], year(parse_date_time(sub_NDA_info$dob[1], order=c('mdy'))))
  ...
  cbind(get(guid_colnames[2]),get(guid_colnames[3]),get(guid_colnames[4]),get(guid_colnames[5]),get(guid_colnames[6]),
  ...
  if (numethnicity<2 & (nrow(subethnicity)==1)){
    race=ifelse(subethnicity$item=="Ethnicity_1","Black or African American",
                     ifelse(subethnicity$item=="Ethnicity_2","Unknown or not reported",
                            ifelse(subethnicity$item=="Ethnicity_3","American Indian/Alaska Native Asian",
...
ndar_ethnicity<-lapply(as.list(unique(ethnicity$tagid)),figure_out_ethnicity)
...
participants_ndar_data <- redcap_cleaned %>%
  ...
  mutate(src_subject_id=tagid,
         ...
         study="UO TAG Study")
participants_ndar_data<-left_join(ndar_race,participants_ndar_data, by="tagid")
...
ndar_bfi10_data <- ndar_subject %>%
  select(subjectkey, src_subject_id, study, site)
ndar_bfi10_data <- ndar_socreward_data %>%
