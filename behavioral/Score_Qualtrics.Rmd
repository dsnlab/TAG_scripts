...
Prepare ERQ (do not submit to RDoC)
```{r}
...
Prepare IFS (do not submit to RDoC)
```{r}
```
...
SSDS <- SSDS %>% filter(!(tagid=="TAG145" & survey_name=="TAG - Home Qs - V3 - with SID embedded"))
...
SSDS_Wave1_totals_graph<-ggplot(SSDS_Wave1_totals, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+
  ggtitle(paste0("Self-Perception Profile for Adolescents total subscores for ",length(unique(SSDS_Wave1$tagid[!is.na(SSDS_Wave1$SSDS_scholasticcomp_total)]))," participants"))

## Make RDOC ready file
ndar_sppa_data <- left_join(SSDS_Wave1,redcap_cleaned,by="tagid") %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
together<-rbind(sppa_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/may2017/harterac01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
rm(together,part2,part3,sppa_df_header,sppa_temp,sppa_temp_df,SSDS,SSDS_Wave1,SSDS_Wave1_means,SSDS_Wave1_means_graph,
