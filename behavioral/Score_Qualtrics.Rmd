...
workdir='/data/jflournoy/a_new_workdir/'
```
...
credsFile<-file.path(workdir,"Questionnaires/Confidential/credentials.yaml.DEFAULT")
creds<-creds_from_file(credsFile)

if(!file.exists(file.path(workdir, 'Questionnaires/Confidential/saved_survey_data.RDS')))
{
  ...
  saveRDS(raw_survey_data, file.path(workdir, 'Questionnaires/Confidential/saved_survey_data.RDS'))
} else {
  raw_survey_data <- readRDS(file.path(workdir, 'Questionnaires/Confidential/saved_survey_data.RDS'))
}
...
write.csv(guid_batch.df,file=paste0(workdir,"RDoCdb/Confidential/GUID_batch_",Sys.Date(),".csv"),row.names = FALSE, quote = F)
write.csv(guid_batch_ids,file=paste0(workdir,"RDoCdb/Confidential/GUID_batch_subIDkey_",Sys.Date(),".csv"),row.names = FALSE, quote = F)
```

Get Survey Date info
```{r}
#add wave to cleaned survey data
cleaned_survey_data <- cleaned_survey_data %>%
    mutate(wave = ifelse(grepl('W2', survey_name), 2, 1))
...
survey_startdate<-filter(cleaned_survey_data, grepl("StartDate",item)) %>%
  ...
  mutate(startdate=as.Date(value),
         survey_type=ifelse(grepl("(Sess 1|W1S1|W2S1)",survey_name),"Sess 1",
                            ifelse(grepl("(Sess 2|W1S2|W2S2)",survey_name),"Sess 2",
...
```{r}
generated_guids <- read.delim(file.path(workdir,'RDoCdb/Confidential/GUID_Batch_Current/output_guid_1515883994322.txt'), header = F, sep = '-') 

generated_psuedo_guids <- read.delim(file.path(workdir,'RDoCdb/Confidential/GUID_Batch_Current/output_pguid_1515883994322.txt'), header = F, sep = '-') 

NDAR_ids <- bind_rows(generated_guids, generated_psuedo_guids)
names(NDAR_ids) <- c('ID', 'guid')
...
ndar_key<-left_join(read.csv(file=paste0(workdir,"RDoCdb/Confidential/GUID_Batch_Current/GUID_batch_subIDkey_2018-01-12.csv"),stringsAsFactors = FALSE),
...
ethnicity_long<-cleaned_survey_data %>%
  ...
  select(-survey_name) %>%
  arrange(tagid) %>%
  distinct(item, value, tagid, wave)

figure_out_ethnicity=function(adf){
  subid <- adf['tagid']
  wave <- adf['wave']
  subethnicity<-ethnicity_long %>%
    filter(tagid==as.character(subid), wave == wave)
...
ndar_ethnicity<-apply(unique(ethnicity_long[,c('tagid', 'wave')]),1,figure_out_ethnicity)
ndar_ethnicity.df<-data.frame(t(ndar_ethnicity),stringsAsFactors=FALSE)

ndar_race<- ndar_ethnicity.df %>%
  mutate(tagid=.[[1]],
         race=.[[2]],
         wave=.[[3]]) %>%
  select(tagid, race, wave) %>%
...
participants_ndar_data <- redcap_cleaned %>%
  ...
  mutate(src_subject_id=tagid,
         ...
         family_study="No",
         sample_taken="Yes",
         sample_id_original=paste0("Session",wave,"_Saliva"),
         ...
         comments_misc=paste0("Wave",wave," Session1"),
         study="UO TAG Study",
         wave)
participants_ndar_data<-left_join(participants_ndar_data,(ndar_race %>%
                                                            filter(tagid %in% ndar_key$tagid)), by=c("tagid","wave")) %>%
...
participants_ndar_data<-left_join(ndar_key,participants_ndar_data, by="tagid") %>%
  mutate(src_subject_id=anontagid,
         subjectkey=guid) %>%
  select(-tagid,-sa_date,-dob,-anontagid,-ID,-guid,-wave) 
...
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/ndar_subject01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
BFI10<-left_join(filter(cleaned_survey_data, grepl("BFI",item)) %>% 
                   ...
                   filter(!is.na(value)) %>% 
                   distinct(tagid,item,value,survey_name,wave,.keep_all = FALSE) %>% 
                   spread(item,value), 
                 redcap_cleaned %>%
                   filter(!is.na(dob),!is.na(sa_date))  %>% mutate(wave=as.numeric(wave)) %>%
                   select(tagid, sa_date, sb_date, dob, wave),by=c("tagid","wave"))

BFI10_Wave12<-left_join(BFI10,survey_date, by = c("tagid","survey_name","wave")) %>% 
...
## Save it
write.csv(BFI10_Wave12, file = paste0(workdir,"Questionnaires/Waves_12/BFI10_Wave12.csv"))

## Graph it
BFI10_Wave12_totals<-BFI10_Wave12 %>%
  select(tagid,wave,BFI10_neuro_total,BFI10_open_total,BFI10_consc_total,BFI10_agree_total,BFI10_extra_total) %>%
...
BFI10_Wave1_totals_graph<-ggplot(BFI10_Wave12_totals, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+
  ggtitle(paste0("BFI Wave1 total for ",length(BFI10_Wave12_totals$tagid[!is.na(BFI10_Wave12_totals$tagid)])," participants")) +
    facet_wrap(~wave, scales='free_x')


BFI10_Wave12_means<-BFI10_Wave12 %>%
  select(tagid,wave,BFI10_neuro_mean,BFI10_open_mean,BFI10_consc_mean,BFI10_agree_mean,BFI10_extra_mean) %>%
  ...
  gather('item','value',3:length(.)) %>%
  filter(!grepl("BFI10",item))
BFI10_Wave12_means_graph<-ggplot(BFI10_Wave12_means, aes(x=value, colour=item)) +
  geom_density(alpha=.3) +
  ggtitle(paste0("BFI Wave1 means for ",length(BFI10_Wave12_means$tagid[!is.na(BFI10_Wave12_means$tagid)])," participants")) +
    facet_wrap(~wave, scales='free_x')

redcap_cleaned <- mutate(redcap_cleaned, wave = as.numeric(wave))

## Make RDOC ready file
ndar_bfi10_data <- left_join(BFI10_Wave12,redcap_cleaned,by=c("tagid","wave")) %>%
...
ndar_bfi10_data<-bind_rows(bfi10_temp_df,ndar_bfi10_data %>% select(-wave))

part2<-colnames(ndar_bfi10_data)
part3<-as.matrix(ndar_bfi10_data)
colnames(part3)<-NULL
together<-rbind(bfi10_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/bfi1001.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
CAMM_Wave1<-left_join(CAMM,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>% 
  select(-value) %>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 1 - V4 - Current",sa_date,
...
## Save it
write.csv(CAMM_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/CAMM_Wave12.csv"))
...
## Make RDOC ready file
ndar_camm_data <- left_join(CAMM_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
         camm_1=CAMM_1,
...
ndar_camm_data<-bind_rows(camm_temp_df,ndar_camm_data %>% select(-wave))

part2<-colnames(ndar_camm_data)
part3<-as.matrix(ndar_camm_data)
colnames(part3)<-NULL
together<-rbind(camm_df_header,part2,part3) 
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/camm01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
rm(CAMM,CAMM_Wave1,CAMM_Wave1_means,CAMM_Wave1_means_graph,CAMM_Wave1_totals,CAMM_Wave1_totals_graph,
...
PDS_Wave1<-left_join(PDS,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>% 
  select(-value) %>%
...
## Save it
write.csv(PDS_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/PDS_Wave12.csv"))
...
options(scipen=999)
PDS_withage<-(left_join(PDS_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
...
## Make RDOC ready file
ndar_pds_data <- left_join(PDS_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
...
anthro <- mutate(anthro, wave = as.numeric(wave))
ndar_pds_data <- left_join(ndar_pds_data,anthro %>% select(tagid,weight,height,wave), by=c("tagid","wave")) %>%
...
ndar_pds_data<-bind_rows(pds_temp_df,ndar_pds_data %>% select(-wave))

part2<-colnames(ndar_pds_data)
part3<-as.matrix(ndar_pds_data)
colnames(part3)<-NULL
together<-rbind(pds_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/pds01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
PBIP_Wave1<-left_join(PBIP,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>% 
  select(-value) %>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
  ...
  filter(!stage=="NaN")
## Save it
write.csv(PBIP_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/PBIP_Wave12.csv"))
...
options(scipen=999)
PBIP_PDS_Wave1<-left_join(PBIP_Wave1,read.csv(paste0(workdir,"Questionnaires/Waves_12/PDS_Wave12.csv"),header=TRUE,stringsAsFactors = FALSE))
...
## Make RDOC ready file
ndar_pbip_data <- left_join(PBIP_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_pbip_data<-bind_rows(pbip_temp_df,ndar_pbip_data %>% select(-wave))

part2<-colnames(ndar_pbip_data)
part3<-as.matrix(ndar_pbip_data)
colnames(part3)<-NULL
together<-rbind(pbip_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/tanner_sms01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
CARE_R_SOC_Wave1<-left_join(CARE_R_SOC,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>% 
  select(-value) %>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(CARE_R_SOC_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/CARE_R_SOC_Wave12.csv"))
...
## Make RDOC ready file
ndar_carersoc_data <- left_join(CARE_R_SOC_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
...
ndar_carersoc_data<-bind_rows(carersoc_temp_df,ndar_carersoc_data %>% select(-wave))

part2<-colnames(ndar_carersoc_data)
part3<-as.matrix(ndar_carersoc_data)
colnames(part3)<-NULL
together<-rbind(carersoc_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/caore01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
CESDC_Wave1<-left_join(CESDC,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>% 
  select(-value) %>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(CESDC_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/CESDC_Wave12.csv"))
...
## Make RDOC ready file
ndar_cesdc_data <- left_join(CESDC_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
...
ndar_cesdc_data<-bind_rows(cesdc_temp_df,ndar_cesdc_data %>% select(-wave))

part2<-colnames(ndar_cesdc_data)
part3<-as.matrix(ndar_cesdc_data)
colnames(part3)<-NULL
together<-rbind(cesdc_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/ces_d01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
CRQ_Wave1<-left_join(CRQ,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>% 
  select(-value) %>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(CRQ_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/CRQ_Wave12.csv"))
...
## Make RDOC ready file
ndar_crq_data <- left_join(CRQ_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_crq_data <- left_join(ndar_key,ndar_crq_data, by="tagid") %>%
  ...
  select(-contains("CRQ",ignore.case = FALSE),-dob,-survey_date,
         -survey_type,-survey_name,-tagid,-anontagid,-ID,-sa_date,-sb_date,-guid)
...
ndar_crq_data<-bind_rows(crq_temp_df,ndar_crq_data %>% select(-wave))

part2<-colnames(ndar_crq_data)
part3<-as.matrix(ndar_crq_data)
colnames(part3)<-NULL
together<-rbind(crq_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/crq01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
CTQ_Wave1<-left_join(CTQ,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>% 
...
## Save it
write.csv(CTQ_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/CTQ_Wave12.csv"))
...
## Make RDOC ready file
ndar_ctq_data <- left_join(CTQ_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_ctq_data<-bind_rows(ctq_temp_df,ndar_ctq_data %>% select(-wave))

part2<-colnames(ndar_ctq_data)
part3<-as.matrix(ndar_ctq_data)
colnames(part3)<-NULL
together<-rbind(ctq_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/ctq01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
DIT_Wave1<-left_join(DIT,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 1 - V4 - Current",sa_date,
...
## Save it
write.csv(DIT_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/DIT_Wave12.csv"))
...
## Make RDOC ready file
ndar_dit_data <- left_join(DIT_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_dit_data<-bind_rows(dit_temp_df,ndar_dit_data %>% select(-wave))

part2<-colnames(ndar_dit_data)
part3<-as.matrix(ndar_dit_data)
colnames(part3)<-NULL
together<-rbind(dit_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/dit01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (length(DSHI$tagid) == dim(unique(DSHI[,c('tagid','wave')]))[1]){
...
DSHI_Wave1<-left_join(DSHI,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
...
## Save it
write.csv(DSHI_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/DSHI_Wave12.csv"))

## Make RDOC ready file
ndar_dshi_data <- left_join(DSHI_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
...
ndar_dshi_data <- left_join(ndar_key,ndar_dshi_data, by="tagid") %>%
  ...
  filter(!is.na(subjectkey),
         !interview_age=="") %>%
  select(-contains("DSHI",ignore.case = FALSE),-dob,-survey_date,-survey_name,-tagid,-anontagid,-ID,-sa_date,-sb_date,-guid,-survey_type)
...
ndar_dshi_data<-bind_rows(dshi_temp_df,ndar_dshi_data %>% select(-wave))

part2<-colnames(ndar_dshi_data)
part3<-as.matrix(ndar_dshi_data)
colnames(part3)<-NULL
together<-rbind(dshi_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/dshi01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
EHI_Wave1<-left_join(EHI,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
...
## Save it
write.csv(EHI_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/EHI_Wave12.csv"))

## Make RDOC ready file
ndar_ehi_data <- left_join(EHI_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_ehi_data<-bind_rows(ehi_temp_df,ndar_ehi_data %>% select(-wave))

part2<-colnames(ndar_ehi_data)
part3<-as.matrix(ndar_ehi_data)
colnames(part3)<-NULL
together<-rbind(ehi_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/edinburgh_hand01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
ERQ_Wave1<-left_join(ERQ,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 1 - V1",sa_date,
...
## Save it
write.csv(ERQ_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/ERQ_Wave1.csv"))
...
## Make RDOC ready file
ndar_erq_data <- left_join(ERQ_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_erq_data<-bind_rows(erq_temp_df,ndar_erq_data %>% select(-wave))

part2<-colnames(ndar_erq_data)
part3<-as.matrix(ndar_erq_data)
colnames(part3)<-NULL
together<-rbind(erq_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/emrq01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (length(IFS$tagid) == dim(unique(IFS[,c('tagid','wave')]))[1]){
  print("No duplicate TAGIDs in data")
  } else {
      print("DUPLICATE TAGIDs in data")
  }
...
IFS <- IFS %>%
    filter(!(survey_name == 'TAG - W1S2 - Current V3' & tagid == 'TAG034'))
...
IFS_Wave1<-left_join(IFS,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(IFS_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/IFS_Wave1.csv"))
...
## Make RDOC ready file
ndar_ifs_data <- left_join(IFS_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_ifs_data<-bind_rows(ifs_temp_df,ndar_ifs_data %>% select(-wave))

part2<-colnames(ndar_ifs_data)
part3<-as.matrix(ndar_ifs_data)
colnames(part3)<-NULL
together<-rbind(ifs_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/ifs01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
IRI_Wave1<-left_join(IRI,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(IRI_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/IRI_Wave1.csv"))
...
## Make RDOC ready file
ndar_iri_data <- left_join(IRI_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_iri_data<-bind_rows(iri_temp_df,ndar_iri_data %>% select(-wave))

part2<-colnames(ndar_iri_data)
part3<-as.matrix(ndar_iri_data)
colnames(part3)<-NULL
together<-rbind(iri_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/iri01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
KSRQ<-left_join(filter(cleaned_survey_data, grepl("K_SRQ",item)) %>%
                  ...
                  filter(!is.na(value)) %>%
                  filter(!value=="") %>%
                  distinct(tagid,item,value,wave,.keep_all = FALSE) %>% 
                  mutate(survey_name="Sess 1") %>%
                  filter(!(tagid == 'TAG081' & item %in% c('K_SRQ_2', 'K_SRQ_6') & value == '6')) %>%
                  spread(item,value),
                redcap_cleaned %>%
                  filter(!is.na(dob),!is.na(sa_date)) %>%
                  select(tagid, sa_date, sb_date, dob,wave),by=c("tagid","wave"))

KSRQ_Wave1<-left_join(KSRQ,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=sa_date) %>%
  select(-qualtrics_date,-sb_date,-sa_date,-dob) %>%
...
## Save it
write.csv(KSRQ_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/KSRQ_Wave1.csv"))
...
## Make RDOC ready file
ndar_ksrq_data <- left_join(KSRQ_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_ksrq_data<-bind_rows(ksrq_temp_df,ndar_ksrq_data %>% select(-wave))

part2<-colnames(ndar_ksrq_data)
part3<-as.matrix(ndar_ksrq_data)
colnames(part3)<-NULL
together<-rbind(ksrq_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/srq01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
MSPSS_Wave1<-left_join(MSPSS,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 1 - V4 - Current",sa_date,
...
## Save it
write.csv(MSPSS_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/MSPSS_Wave1.csv"))
...
## Make RDOC ready file
ndar_mspss_data <- left_join(MSPSS_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_mspss_data<-bind_rows(mspss_temp_df,ndar_mspss_data %>% select(-wave))

part2<-colnames(ndar_mspss_data)
part3<-as.matrix(ndar_mspss_data)
colnames(part3)<-NULL
together<-rbind(mspss_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/mspss01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (sum(duplicated(select(PSS, tagid, wave))) == 0){
  print("No duplicate tagids in data")
}

PSS <- PSS %>%
    filter(!(survey_name == 'TAG - Sess 2 - V2' & tagid == 'TAG055'))

PSS_Wave1<-left_join(PSS,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 1 - V4 - Current",sa_date,
...
## Save it
write.csv(PSS_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/PSS_Wave1.csv"))
...
## Make RDOC ready file
ndar_pss_data <- left_join(PSS_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_pss_data <- left_join(ndar_key,ndar_pss_data, by="tagid") %>%
  ...
  filter(!is.na(subjectkey),
         !interview_age=="") %>%
  select(-contains("PSS",ignore.case = FALSE),-dob,-survey_date,-survey_name,-tagid,-anontagid,-ID,-sa_date,-sb_date,-guid,-survey_type)
...
ndar_pss_data<-bind_rows(pss_temp_df,ndar_pss_data %>% select(-wave))

part2<-colnames(ndar_pss_data)
part3<-as.matrix(ndar_pss_data)
colnames(part3)<-NULL
together<-rbind(pss_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/pss01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (sum(duplicated(select(RMET, tagid, wave))) == 0){
  print("No duplicate TAGIDs in data")
}

RMET_Wave1<-left_join(RMET,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(RMET_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/RMET_Wave1.csv"))
...
## Make RDOC ready file
ndar_rmet_data <- left_join(RMET_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_rmet_data<-bind_rows(rmet_temp_df,ndar_rmet_data %>% select(-wave))

part2<-colnames(ndar_rmet_data)
part3<-as.matrix(ndar_rmet_data)
colnames(part3)<-NULL
together<-rbind(rmet_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/rmet_cv01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (sum(duplicated(select(RSCS, tagid, wave))) == 0){
  print("No duplicate TAGIDs in data")
  } else {
      print("DUPLICATE TAGIDs in data")
  }
...
RSCS_Wave1<-left_join(RSCS,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 1 - V4 - Current",sa_date,
...
## Save it
write.csv(RSCS_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/RSCS_Wave1.csv"))
...
## Make RDOC ready file
ndar_rscs_data <- left_join(RSCS_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_rscs_data<-bind_rows(rscs_temp_df,ndar_rscs_data %>% select(-wave))

part2<-colnames(ndar_rscs_data)
part3<-as.matrix(ndar_rscs_data)
colnames(part3)<-NULL
together<-rbind(rscs_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/rscsc01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (sum(duplicated(select(SAQ, tagid, wave))) == 0){
  print("No duplicate TAGIDs in data")
  } else {
      print("DUPLICATE TAGIDs in data")
  }
...
SAQ_Wave1<-left_join(SAQ,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=
...
## Save it
write.csv(SAQ_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/SAQ_Wave1.csv"))
...
## Make RDOC ready file
ndar_saq_data <- left_join(SAQ_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_saq_data<-bind_rows(saq_temp_df,ndar_saq_data %>% select(-wave))

part2<-colnames(ndar_saq_data)
part3<-as.matrix(ndar_saq_data)
colnames(part3)<-NULL
together<-rbind(saq_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/sag01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (sum(duplicated(select(SCARED,tagid,wave))) == 0){
  print("No duplicate TAGIDs in data")
  } else {
      print("DUPLICATE TAGIDs in data")
  }

SCARED_Wave1<-left_join(SCARED,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V2",sb_date,
...
## Save it
write.csv(SCARED_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/SCARED_Wave1.csv"))
...
## Make RDOC ready file
ndar_scared_data <- left_join(SCARED_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_scared_data<-bind_rows(scared_temp_df,ndar_scared_data %>% select(-wave))

part2<-colnames(ndar_scared_data)
part3<-as.matrix(ndar_scared_data)
colnames(part3)<-NULL
together<-rbind(scared_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/scared01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (sum(duplicated(select(SCC,tagid,wave))) == 0){
  print("No duplicate TAGIDs in data")
  } else {
      print("DUPLICATE TAGIDs in data")
  }
...
SCC_Wave1<-left_join(SCC,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(SCC_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/SCC_Wave1.csv"))
...
## Make RDOC ready file
ndar_scc_data <- left_join(SCC_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_scc_data<-bind_rows(scc_temp_df,ndar_scc_data %>% select(-wave))

part2<-colnames(ndar_scc_data)
part3<-as.matrix(ndar_scc_data)
colnames(part3)<-NULL
together<-rbind(scc_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/scc01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (sum(duplicated(select(SPPA,tagid,wave))) == 0){
  print("No duplicate TAGIDs in data")
  } else {
      print("DUPLICATE TAGIDs in data")
  }
...
SPPA_Wave1<-left_join(SPPA,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(SPPA_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/SPPA_Wave1.csv"))
...
## Make RDOC ready file
ndar_sppa_data <- left_join(SPPA_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_sppa_data<-bind_rows(sppa_temp_df,ndar_sppa_data %>% select(-wave))

part2<-colnames(ndar_sppa_data)
part3<-as.matrix(ndar_sppa_data)
colnames(part3)<-NULL
together<-rbind(sppa_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/harterac01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (sum(duplicated(select(SSDS,tagid,wave))) == 0){
  print("No duplicate TAGIDs in data")
  } else {
      print("DUPLICATE TAGIDs in data")
  }
...
SSDS <- SSDS %>%
    filter(!(tagid == 'TAG034' & survey_name == 'TAG - W1S2 - Current V3'))
...
SSDS_Wave1<-left_join(SSDS,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(SSDS_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/SSDS_Wave1.csv"))
...
## Make RDOC ready file
ndar_ssds_data <- left_join(SSDS_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_ssds_data<-bind_rows(ssds_temp_df,ndar_ssds_data %>% select(-wave))

part2<-colnames(ndar_ssds_data)
part3<-as.matrix(ndar_ssds_data)
colnames(part3)<-NULL
together<-rbind(ssds_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/ssds01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
if (sum(duplicated(select(PANAS, tagid, wave)))==0){
  print("No duplicate TAGIDs in data")
  } else {
      print("DUPLICATE TAGIDs in data")
  }
...
PANAS_Wave1<-left_join(PANAS,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
...
## Save it
write.csv(PANAS_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/PANAS_Wave1.csv"))
...
## Make RDOC ready file
ndar_panas_data <- left_join(PANAS_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_panas_data <- left_join(ndar_key,ndar_panas_data, by="tagid") %>%
  ...
  filter(!is.na(subjectkey),
         !interview_age=="") %>%
  select(-contains("PANAS",ignore.case = FALSE),-dob,-survey_date,-survey_name,-tagid,-anontagid,-ID,-sa_date,-sb_date,-guid,-survey_type)
...
ndar_panas_data<-bind_rows(panas_temp_df,ndar_panas_data %>% select(-wave))

part2<-colnames(ndar_panas_data)
part3<-as.matrix(ndar_panas_data)
colnames(part3)<-NULL
together<-rbind(panas_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/panas_c01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
Prepare HAIR # Do not submit to RDOC
```{r}
```
...
if (sum(duplicated(select(CBCL,tagid,wave))) == 0){
  print("No duplicate TAGIDs in data")
  } else {
      print("DUPLICATE TAGIDs in data")
  }

CBCL_Wave1<-left_join(CBCL,survey_date, by = c("tagid","survey_name","wave")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>%
  select(-value)%>%
...
## Save it
write.csv(CBCL_Wave1, file = paste0(workdir,"Questionnaires/Waves_12/CBCL_Wave1.csv"))


## Make RDOC ready file
ndar_cbcl_data <- left_join(CBCL_Wave1,redcap_cleaned,by=c("tagid","wave")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>%
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         interview_age=round((interval(start =dob, end = survey_date) / duration(num = 1, units = "months")),0),
         gender="F",
...
ndar_cbcl_data <- left_join(ndar_key,ndar_cbcl_data, by="tagid") %>%
  ...
  filter(!is.na(subjectkey),
         !interview_age=="") %>%
  select(-contains("CBCL",ignore.case = FALSE),-dob,-survey_date,-survey_name,-tagid,-anontagid,-ID,-sa_date,-sb_date,-guid,-survey_type)
...
ndar_cbcl_data<-bind_rows(cbcl_temp_df,ndar_cbcl_data %>% select(-wave))[c(1:429)]

part2<-colnames(ndar_cbcl_data)
part3<-as.matrix(ndar_cbcl_data)
colnames(part3)<-NULL
together<-rbind(cbcl_df_header,part2,part3)
write.table(together,file =paste0(workdir,"RDoCdb/output/jan2018/cbcl01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
qualtricsData_raw_parent_pds <-filter(qualtricsData_raw_long, grepl("Parent",survey_name)) %>% 
  ...
  mutate(value=as.numeric(value)) %>% 
  filter(!is.na(value)) %>% 
  distinct(tagid,item,value,wave,.keep_all = TRUE) %>% 
  spread(item,value)
