...
redcap_anthro <- read.csv(paste0(workdir,"Questionnaires/Confidential/redcap_anthro.csv"), header = TRUE, stringsAsFactors = FALSE) %>%
  ...
  mutate(tagid=sprintf("TAG%03d",as.integer(tagid))) %>%
  select(-subject_spit_id)
anthro<-left_join(redcap_cleaned,redcap_anthro,by="tagid") %>%
...
CTQ_Wave1<-left_join(CTQ,survey_date, by = c("tagid","survey_name")) %>% 
  distinct(tagid,survey_name,.keep_all = TRUE) %>%
  mutate(qualtrics_date=value) %>% 
  select(-item, -value, -contains("TEXT")) %>%
  mutate(survey_date=ifelse(survey_name=="TAG - Sess 2 - V1",sb_date,
                            ifelse(survey_name=="TAG - Sess 2 - V2",sb_date,
                                   ifelse(survey_name=="TAG - Sess 2 - V3 - Current",sb_date,
                                          ifelse(survey_name=="Sensitive Q's for 042",sb_date,as.character(qualtrics_date)
  ...
  mutate(CTQ_1=CTQ_1..1,
         ...
         CTQ_mean=round(rowMeans(cbind(CTQ_1,CTQ_2,CTQ_3,CTQ_4,CTQ_5,CTQ_6,CTQ_7,CTQ_8,CTQ_9,CTQ_10,CTQ_11,CTQ_12,CTQ_13,CTQ_14,CTQ_15,CTQ_16,CTQ_17,CTQ_18,CTQ_19,CTQ_20,CTQ_21,CTQ_22,CTQ_23,CTQ_24,CTQ_25,CTQ_26,CTQ_27,CTQ_28),na.rm=T),3),
...
## Graph it
CTQ_Wave1_totals_graph<-ggplot(CTQ_Wave1, aes(x=CTQ_total, colour="deeppink")) +
  geom_density(alpha=.3,show.legend = FALSE)+
  ggtitle(paste0("CTQ total scores for ",length(CTQ_Wave1$CTQ_total[!is.na(CTQ_Wave1$CTQ_total)])," Wave 1 participants"))

CTQ_Wave1_means_graph<-ggplot(CTQ_Wave1, aes(x=CTQ_mean, colour="dodgerblue")) +
  geom_density(alpha=.3, show.legend = FALSE)+
  ggtitle(paste0("CTQ mean scores for ",length(CTQ_Wave1$CTQ_mean[!is.na(CTQ_Wave1$CTQ_mean)])," Wave 1 participants"))
...
ndar_crq_data <- left_join(ndar_key,ndar_crq_data, by="tagid") %>%
  ...
  filter(!is.na(subjectkey),
         !interview_age=="") %>%
  select(-contains("CTQ",ignore.case = FALSE),-dob,-survey_date,-survey_name,-tagid,-anontagid,-ID,-sa_date,-sb_date,-guid)
crq_df_header<-rep(NA,length(read.csv(paste0(workdir,"RDoCdb/templates/crq01_template.csv"), header = FALSE, stringsAsFactors = FALSE)))
crq_df_header[1]<-read.csv(paste0(workdir,"RDoCdb/templates/crq01_template.csv"), header = FALSE, stringsAsFactors = FALSE)[1,1]
crq_df_header[2]<-read.csv(paste0(workdir,"RDoCdb/templates/crq01_template.csv"), header = FALSE, stringsAsFactors = FALSE)[1,2]
