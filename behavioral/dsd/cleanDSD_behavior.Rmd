...
longDF <- lapply(X=dsdFiles$dsdFiles, FUN=function(file) {
  ...
  DF.raw <- DF.raw %>%
    ...
    mutate(disclosed=ifelse(target.choice==0,NA,
           ...
           share.value=ifelse(target.choice==0,NA,
                              ifelse(side==1, (right.coin-left.coin),
...
files <- lapply(X=ids, df=out_df, FUN=function(id, df) {
  ...
  write.table(df,file=paste0(outDataDir,'subjects/tag',fid,'.csv'),sep=",",row.names=F,col.names=F,na = "") 
   })
