...
PubertyComposite <- Qdf %>% select(SID,adrenf2,gonadf2,pdss,pdss2,PBIP_1A,PBIP_2A,PBIP_stage) %>%
...
PubertyMissing <- PubertyComposite %>% filter(is.na(PUBcomp_trunc)) %>%
  left_join(.,PDS_parent) #3 kids with missing composite scores have parent PDS data. need to decide if we will incorporate them.
```
