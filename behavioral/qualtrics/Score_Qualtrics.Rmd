...
workdir='/Volumes/psych-cog/dsnlab/TAG/behavior/' #workdir="I:/TAG/behavior/" #(for Sam)
```
...
redcapData <- read.csv(paste0(workdir,"Questionnaires/RDOC/Confidential/redcap_dates.csv"), header = TRUE, stringsAsFactors = FALSE)# %>%
...
write.csv(CRQ_Wave1, file = paste0(workdir,"Questionnaires/Wave1/CRQ_Wave1.csv"))
...
## Save it
write.csv(DIT_Wave1, file = paste0(workdir,"Questionnaires/Wave1/DIT_Wave1.csv"))
...
# Changing W1 Items
updates <- read.csv(paste0(workdir,"Questionnaires/Modified_Answers.csv"))
#DSHI_Wave1_Updated <- left_join(DSHI_Wave1, )
...
KSRQ <- left_join(filter(cleaned_survey_data, grepl("K_SRQ", item)) %>%
                    ...
                    filter(!is.na(value)) %>%
                    distinct(tagid,item,value,survey_name,.keep_all = FALSE) %>% 
                    filter(survey_name!="W2S1 - Current V1") %>% 
                    spread(item, value),
                  redcap_cleaned %>%
                  filter(!is.na(dob),!is.na(sa_date)) %>%
                  select(tagid, sa_date, sb_date, dob),by="tagid")
...
## Save it
write.csv(PSS_Wave1, file = paste0(workdir,"Questionnaires/Wave1/PSS_Wave1.csv"))
...
## Save it
write.csv(RMET_Wave1, file = paste0(workdir,"Questionnaires/Wave1/RMET_Wave1.csv"))
...
Prepare R-SCS-C
```{r}
#duplicate identifiers error - needs to be fixed
...
## Save it
write.csv(SAQ_Wave1, file = paste0(workdir,"Questionnaires/Wave1/SAQ_Wave1.csv"))
...
## Save it
write.csv(SCARED_Wave1, file = paste0(workdir,"Questionnaires/Wave1/SCARED_Wave1.csv"))
...
## Save it
write.csv(SCC_Wave1, file = paste0(workdir,"Questionnaires/Wave1/SCC_Wave1.csv"))
...
## Save it
write.csv(PANAS_Wave1, file = paste0(workdir,"Questionnaires/Wave1/PANAS_Wave1.csv"))
...
## Save it
write.csv(CBCL_Wave1, file = paste0(workdir,"Questionnaires/Wave1/CBCL_Wave1.csv"))
