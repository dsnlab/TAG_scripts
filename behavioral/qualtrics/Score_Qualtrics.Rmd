...
exclude_survey=c("NV","GUT","TEST","Appointment","Newsletter","SOS","Sharing Task","PMS Interest")
...
redcapData <- read.csv(paste0(workdir,"Questionnaires/RDOC/Confidential/redcap_dates_20191213.csv"), header = TRUE, stringsAsFactors = FALSE)# %>%
...
```{r Load&Clean Redcap, message=FALSE, warning=FALSE, include=FALSE}
redcap_anthro <- read.csv(paste0(workdir,"Questionnaires/RDOC/Confidential/redcap_anthro_20191213.csv"), header = TRUE, stringsAsFactors = FALSE) %>%
  filter(!anthro_doc=="") %>%
  extract(redcap_event_name, c('wave'), 'wave_([\\d]).*') %>%
  mutate(tagid=substring(subject_spit_id,first=4,last=length(subject_spit_id)))%>%
  filter(!subject_spit_id=="TAG_001P") %>%
...
redcap_NDA_info <- read.csv(paste0(workdir,"RDoCdb/Confidential/redcap_nda_consent_20191213.csv"),
                            header = TRUE,
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/ndar_subject01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
