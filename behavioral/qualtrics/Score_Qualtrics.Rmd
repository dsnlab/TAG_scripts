...
#Need to do a little cleaning before we don anything else.
...
redcapData <- read.csv(paste0(workdir,"Questionnaires/RDOC/Confidential/redcap_dates_20220210.csv"), header = TRUE, stringsAsFactors = FALSE)# %>%
...
```{r Load&Clean Redcap, message=FALSE, warning=FALSE, include=FALSE}
redcap_anthro <- read.csv(paste0(workdir,"Questionnaires/RDOC/Confidential/redcap_anthro_20220210.csv"), header = TRUE, stringsAsFactors = FALSE) %>%
  filter(!anthro_doc=="") %>%
  extract(redcap_event_name, c('wave'), 'wave_([\\d]).*') %>%
  mutate(tagid=substring(subject_spit_id,first=4,last=length(subject_spit_id)))%>%
  filter(!subject_spit_id=="TAG_001P") %>%
...
```{r}
survey_startdate<-filter(cleaned_survey_data, grepl("time_PDS",item)) %>%
...
PDS_Wave3_outdf <- PDS_Wave1 %>% filter(grepl("W3",survey_name))
