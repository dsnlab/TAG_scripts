...
author: "Kate, Cameron, Nandi, John, & Sam"
date: "10 Feb 2022"
...
raw_survey_data <- get_survey_data(survey_info, pid_cols=c("RecipientLastName", "SID_embed", "SID"))
...
raw_survey_data[SID == "SV_eqvWqPtsOO4uqPP", SID := NA] #SJC updated #Not strictly necessary since `fix_ids` would do this too.
...
#explanatory.
raw_survey_data$SID <- scorequaltrics::fix_ids(raw_survey_data, pid_cols=c("RecipientLastName", "SID_embed", "SID"), 
...
cleaned_survey_data <- as.data.table(long_survey_data) %>% select(SID, item, value, survey_name)
...
for (i in 1:length(misIDkey$qid)){
  TAGHomeQ <- TAGHomeQ %>%
    mutate(value=ifelse(ResponseId==as.character(misIDkey$qid[i]) && item=="SID",
...
test1 <- select(TAGEmbeddedHomeQ_Gathered, tagid, survey_name, ResponseId, everything())
...
redcapData <- read.csv(paste0(workdir,"Questionnaires/RDOC/Confidential/redcap_dates_20220427.csv"), header = TRUE, stringsAsFactors = FALSE)# %>%
...
```{r Load&Clean Redcap, message=FALSE, warning=FALSE, include=FALSE}
redcap_anthro <- read.csv(paste0(workdir,"Questionnaires/RDOC/Confidential/redcap_anthro_20220427.csv"), header = TRUE, stringsAsFactors = FALSE) %>%
  filter(!anthro_doc=="") %>%
  extract(redcap_event_name, c('wave'), 'wave_([\\d]).*') %>%
  mutate(tagid=substring(subject_spit_id,first=4,last=length(subject_spit_id)))%>%
  filter(!subject_spit_id=="TAG_001P") %>%
...
redcap_NDA_info <- read.csv(paste0(workdir,"RDoCdb/Confidential/redcap_nda_consent_20220210.csv"),
                            header = TRUE,
...
survey_startdate<-filter(cleaned_survey_data, grepl("StartDate",item)) %>%
  ...
  mutate(startdate=as.Date(value),
         survey_type=ifelse(grepl("(Sess 1|W[1234]S1)",survey_name),"Sess 1",
                            ifelse(grepl("(Sess 2|W[1234]S2)",survey_name),"Sess 2",
...
ndar_subject_df<-data.frame(lapply(ndar_subject, as.character), stringsAsFactors = FALSE) %>%
  mutate(interview_age = as.double(interview_age))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/feb2022/ndar_subject01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
BFI10 <- filter(cleaned_survey_data, grepl("BFI",item)) %>% 
                   mutate(value=as.numeric(value)) %>% 
                   filter(!is.na(value)) %>% 
                   distinct(tagid,item,value,survey_name,.keep_all = FALSE)
...
#fix coding
BFI10_test <- filter(cleaned_survey_data, grepl("BFI",item)) %>% 
                   filter(!is.na(value)) %>% 
                   distinct(tagid,item,survey_name,.keep_all = T)
BFI10_test["value"][BFI10_test["value"] == "Disagree strongly"] <- "1"
BFI10_test["value"][BFI10_test["value"] == "Disagree a little"] <- "2"
BFI10_test["value"][BFI10_test["value"] == "Neither agree nor disagree"] <- "3"
BFI10_test["value"][BFI10_test["value"] == "Agree a little"] <- "4"
BFI10_test["value"][BFI10_test["value"] == "Agree strongly"] <- "5"

date_test <- redcap_cleaned %>%
  filter(!is.na(dob),!is.na(sa_date)) 

BFI10<-left_join(date_test %>%
                   select(tagid, sa_date, sb_date, dob, wave),
                 BFI10_test %>% 
                   #mutate(value=as.numeric(value)) %>% 
                   spread(item,value),by = "tagid")
cols.num <- c(7:16)
BFI10[cols.num] <- sapply(BFI10[cols.num],as.numeric)
sapply(BFI10, class)

BFI10 <- BFI10 %>% mutate(survey_date=ifelse(is.na(`time_BFI-10`),
                                      paste0(sprintf("%02d",month(sa_date)),"/",sprintf("%02d",day(sa_date)),"/",year(sa_date)),
                                      paste0(sprintf("%02d",month(`time_BFI-10`)),"/",sprintf("%02d",day(`time_BFI-10`)),"/",year(`time_BFI-10`)))) 



#BFI10_Wave1<-left_join(BFI10,survey_originaldate, by = c("tagid","survey_name")) %>% 
 # distinct(tagid,survey_name,.keep_all = T) %>%
  #mutate(qualtrics_date=ifelse(!is.na(originaldate), sa_date, originaldate))

#BFI10_Wave1<-left_join(BFI10,survey_originaldate, by = c("tagid","survey_name")) %>% 
 # distinct(tagid,survey_name,.keep_all = T) %>%
  #mutate(qualtrics_date=originaldate) %>% 
  #select(-originaldate) %>%
 ...
 #                           ifelse(survey_name=="TAG - Sess 1 - V1",sa_date,
  #                                 ifelse(survey_name=="TAG - Sess 1 - V2",sa_date,
   #                                       ifelse(survey_name=="TAG - Sess 1 - V3",sa_date,
    #                                             ifelse(survey_name=="Sensitive Q's for 042",sb_date,as.character(`time_BFI-10`)
     #                                            )))))) %>%
#  select(-originaldate,-sb_date,-sa_date,-dob) %>%
 # distinct(tagid,survey_name,.keep_all = TRUE) %>%
BFI10_Wave1 <- BFI10 %>%
  distinct(tagid,survey_name,survey_date,.keep_all = T) %>%
  select(-wave,-sb_date,-sa_date,-dob, -`time_BFI-10`) %>%
...
#tidying
#BFI10_Wave1 <- BFI10_Wave1[,c(2,1,3:12,15:40)] 
BFI10_Wave1 <- BFI10_Wave1[order(BFI10_Wave1$tagid), ]

## Save it
BFI10_Wave1_outdf <- BFI10_Wave1 %>% filter(!grepl("W2|W3|W4",survey_name)) 
...
write.csv(BFI10_Wave4_outdf, file = paste0(workdir,"Questionnaires/Wave4/BFI10_Wave4.csv"))
...
BFI10_Wave1_totals_graph<-ggplot(BFI10_Wave1_totals, aes(x=value, colour=item)) +
  ...
  ggtitle(paste0("BFI Wave1 total for ",length(BFI10_Wave1_outdf$tagid[!is.na(BFI10_Wave1_outdf$tagid)])," participants"))
...
BFI10_Wave2_totals_graph<-ggplot(BFI10_Wave2_totals, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+
  ggtitle(paste0("BFI Wave2 total for ",length(BFI10_Wave2_outdf$tagid[!is.na(BFI10_Wave2_outdf$tagid)])," participants"))
...
BFI10_Wave3_totals_graph<-ggplot(BFI10_Wave3_totals, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+
  ggtitle(paste0("BFI Wave2 total for ",length(BFI10_Wave3_outdf$tagid[!is.na(BFI10_Wave3_outdf$tagid)])," participants"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/bfi1001.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
Prepare CAMM
```{r}
CAMM<-left_join(CAMM <- filter(cleaned_survey_data, grepl("CAMM",item)) %>% 
                  mutate(value=as.numeric(value)) %>% 
                  filter(!is.na(value)) %>% 
                  filter(!grepl("TAG160",tagid) | grepl("W2|W3",survey_name)) %>% 
  #need to readd TAG160 at Wave 1 when we decide which answers we're including
                  distinct(tagid,item,value,survey_name,.keep_all = FALSE) %>%                                    spread(item,value),
                redcap_cleaned %>%
                  filter(!is.na(dob),!is.na(sa_date)) %>%
                  select(tagid, sa_date, sb_date, dob),by="tagid")
...
write.csv(CAMM_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave3/CAMM_Wave3.csv"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/camm01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
PDS <- PDS %>%
  mutate(PDS_F6 = ifelse((!grepl("W2|W3|W4",survey_name) & is.na(PDS_F6) & (tagid %in% w1_Menarche$tagid)), 1, 
                         ifelse((!grepl("W2|W3|W4",survey_name) & is.na(PDS_F6) & (tagid %in% w1_NoMenarche$tagid)), 0,
                                PDS_F6)))
...
PDS_cleaned <- PDS_Wave1 %>% mutate(PDS_F7 = 
  ...
  select(-c(PDS_F10_1_1,PDS_F10_1_2,PDS_F10_1_4))

## Save it
PDS_Wave1_outdf <- PDS_cleaned %>% filter(!grepl("W2|W3|W4",survey_name)) #NV: given comment above, I'm now saving another filtered PDS_wave1 document for saving output.
...
write.csv(PDS_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave3/PDS_Wave3.csv"))

PDS_Wave4_outdf <- PDS_cleaned %>% filter(grepl("W4",survey_name))
write.csv(PDS_Wave4_outdf, file = paste0(workdir,"Questionnaires/Wave4/PDS_Wave4.csv"))
...
PDS_Wave2_pdss_graph<-ggplot(PDS_Wave2_outdf, aes(x=pdss, colour="deeppink")) +
  geom_density(alpha=.3)+
  ggtitle(paste0("PDSS scores for ",length(PDS_Wave2_outdf$pdss[!is.na(PDS_Wave2_outdf$pdss)])," Wave 1 participants"))

PDS_Wave3_pdss_graph<-ggplot(PDS_Wave3_outdf, aes(x=pdss, colour="deeppink")) +
  geom_density(alpha=.3)+
  ggtitle(paste0("PDSS scores for ",length(PDS_Wave3_outdf$pdss[!is.na(PDS_Wave3_outdf$pdss)])," Wave 1 participants"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/pds01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
## Save it
PBIP_Wave1_outdf <- PBIP_Wave1 %>% filter(!grepl("W2|W3|W4",survey_name)) #NV: Same comment as PDS re filtering to wave 1.
...
write.csv(PBIP_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave3/PBIP_Wave3.csv"))

PBIP_Wave4_outdf <- PBIP_Wave1 %>% filter(grepl("W4",survey_name))
write.csv(PBIP_Wave4_outdf, file = paste0(workdir,"Questionnaires/Wave4/PBIP_Wave4.csv"))
...
options(scipen=999)
PBIP_PDS_Wave1<-left_join(PBIP_Wave1_outdf,read.csv(paste0(workdir,"Questionnaires/Wave1/PDS_Wave1.csv"),header=TRUE,stringsAsFactors = FALSE))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/tanner_sms01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/caore01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
## Save it
CESDC_Wave1_outdf <- CESDC_Wave1 %>% filter(!grepl("W2|W3|W4",survey_name)) 
...
write.csv(CESDC_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave3/CESDC_Wave3.csv"))

CESDC_Wave4_outdf <- CESDC_Wave1 %>% filter(grepl("W4",survey_name)) 
write.csv(CESDC_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave4/CESDC_Wave4.csv"))
...
require(gridExtra)
grid.arrange(CESDC_Wave1_totals_graph, CESDC_Wave2_totals_graph, CESDC_Wave3_totals_graph, ncol=3)

cesdc_change_12 <- merge(CESDC_Wave1_outdf, CESDC_Wave2_outdf, by = "tagid", all = T)
cesdc_change_12 <- cesdc_change_12[c("tagid","CES_DC_total.x","CES_DC_total.y")]
cesdc_change_13 <- merge(CESDC_Wave1_outdf, CESDC_Wave3_outdf, by = "tagid", all = T)
cesdc_change_13 <- cesdc_change_13[c("tagid","CES_DC_total.x","CES_DC_total.y")]
...
cesdc_long <- gather(cesdc_change_12, wave, ces_dc_total, CES_DC_total.x:CES_DC_total.y, factor_key=TRUE)
cesdc_long$wave <- ifelse(cesdc_long$wave=="CES_DC_total.x",1,2)

tspag = ggplot(cesdc_long, aes(x=wave, y=ces_dc_total)) + 
  geom_line() + guides(colour=FALSE) + xlab("Wave") +
  ylab("CES-DC-Total Score")
spag = tspag + aes(colour = factor(tagid)) + scale_x_continuous(breaks=c(1,2)) + ylim(0,45)
spag2 <- spag + geom_smooth(se=FALSE, colour="black", size=2)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/ces_d01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/crq01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
CTQ_Wave1<-left_join(CTQ,survey_date, by = c("tagid","survey_name")) %>% 
  ...
  select(-qualtrics_date,-sb_date,-sa_date,-dob) %>% 
  select(-CTQ_1_2_1, -CTQ_2_2_1, -CTQ_3_2_1, -CTQ_4_2_1, -CTQ_5_2_1, -CTQ_6_2_1, 
...
## Save it
CTQ_Wave1_outdf <- CTQ_Wave1 %>% filter(!grepl("W2|W3|W4",survey_name)) 
...
write.csv(CTQ_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave3/CTQ_Wave3.csv"))

CTQ_Wave4_outdf <- CTQ_Wave1 %>% filter(grepl("W4",survey_name)) 
write.csv(CTQ_Wave4_outdf, file = paste0(workdir,"Questionnaires/Wave4/CTQ_Wave4.csv"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/ctq01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/dit01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.csv(DSHI_Wave1_outdf, file = paste0(workdir,"Questionnaires/Wave1/DSHI_Wave1.csv"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/edinburgh_hand01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/emrq01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
IFS_cleaned<-left_join(IFS,survey_date, by = c("tagid","survey_name")) %>% 
  ...
  select(-sb_date,-sa_date,-dob, -qualtrics_date) %>%
...
## Save it
IFS_Wave1_outdf <- IFS_cleaned %>% filter(!grepl("W2|W3|W4",survey_name)) 
...
write.csv(IFS_Wave4_outdf, file = paste0(workdir,"Questionnaires/Wave4/IFS_Wave4.csv"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/ifs01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/iri01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/srq01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
## Save it
MSPSS_Wave1_outdf <- MSPSS_Wave1 %>% filter(!grepl("W2|W3|W4",survey_name)) 
...
write.csv(MSPSS_Wave4_outdf, file = paste0(workdir,"Questionnaires/Wave4/MSPSS_Wave4.csv"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/mspss01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
PSS_Wave123_FirstResponse <- PSS_Wave1 %>% filter(!grepl("W3S2",survey_name))

PSS_Wave123_FirstResponse <- PSS_Wave123_FirstResponse %>%
  #select(-qualtrics_date,-sb_date,-sa_date,-dob) %>% # add this line if you are not making RDoC db document
...
## Save it
PSS_Wave1_outdf <- PSS_Wave123_FirstResponse %>% filter(!grepl("W2|W3",survey_name)) 
write.csv(PSS_Wave1_outdf, file = paste0(workdir,"Questionnaires/Wave1/PSS_Wave1.csv"))

PSS_Wave2_outdf <- PSS_Wave123_FirstResponse %>% filter(grepl("W2",survey_name)) 
write.csv(PSS_Wave2_outdf, file = paste0(workdir,"Questionnaires/Wave2/PSS_Wave2.csv"))

PSS_Wave3_Session1_outdf <- PSS_Wave123_FirstResponse %>% filter(grepl("W3",survey_name))
PSS_Wave3_Session2_outdf <- PSS_Wave3_Session2
write.csv(PSS_Wave3_Session1_outdf, file = paste0(workdir,"Questionnaires/Wave3/PSS_Wave3_Session1.csv"))
write.csv(PSS_Wave3_Session2_outdf, file = paste0(workdir,"Questionnaires/Wave3/PSS_Wave3_Session2.csv"))

## Graph it
PSS_Wave1_totals<-PSS_Wave123_FirstResponse %>%
...
PSS_Wave1_totals_graph<-ggplot(PSS_Wave1_totals, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+
  ggtitle(paste0("PSS total score for ",length(unique(PSS_Wave123_FirstResponse$tagid[!is.na(PSS_Wave123_FirstResponse$PSS_total)]))," participants"))

PSS_Wave1_means<-PSS_Wave123_FirstResponse %>%
...
PSS_Wave1_means_graph<-ggplot(PSS_Wave1_means, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+
  ggtitle(paste0("PSS mean score for ",length(unique(PSS_Wave123_FirstResponse$tagid[!is.na(PSS_Wave123_FirstResponse$tagid)]))," participants"))


## Make RDOC ready file
ndar_pss_data <- left_join(PSS_Wave123_FirstResponse,distinct(redcap_cleaned, tagid, dob), by=c("tagid", "dob")) %>%
  filter(!is.na(survey_date),!survey_date=="") %>% 
...
ndar_pss_data <- left_join(ndar_key,ndar_pss_data, by="tagid") %>%
  ...
  select(-contains("PSS",ignore.case = FALSE),-dob,-survey_date,-survey_name,-tagid,-anontagid,-ID,-guid,-survey_type,
         -sa_date,-sb_date,-qualtrics_date)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/pss01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/rmet_cv01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
## Save it
RSCS_Wave1_outdf <- RSCS_Wave1 %>% filter(!grepl("W2|W3|W4",survey_name)) 
...
write.csv(RSCS_Wave4_outdf, file = paste0(workdir,"Questionnaires/Wave4/RSCS_Wave4.csv"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/rscsc01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/sag01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
SCARED_Wave3_outdf <- SCARED_Wave1 %>% filter(grepl("W3",survey_name)) 
write.csv(SCARED_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave3/SCARED_Wave3.csv"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/scared01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
SCC_Wave1_total_graph<-ggplot(SCC_Wave1_total, aes(x=value, colour=item)) +
  ...
  ylim(0,0.05) + geom_vline(aes(xintercept = median(SCC_Wave1_outdf$SCC_total, na.rm=T), 
                                colour="SCC_total"))
...
SCC_Wave2_total_graph<-ggplot(SCC_Wave2_total, aes(x=value, colour=item)) +
  ...
  ylim(0,0.05) + geom_vline(aes(xintercept = median(SCC_Wave2_outdf$SCC_total, na.rm=T), 
                                colour="SCC_total"))
...
SCC_Wave3_total_graph<-ggplot(SCC_Wave3_total, aes(x=value, colour=item)) +
  ...
  ylim(0,0.05) + geom_vline(aes(xintercept = median(SCC_Wave3_outdf$SCC_total, na.rm=T), 
                                colour="SCC_total"))

require(gridExtra)
grid.arrange(SCC_Wave1_total_graph, SCC_Wave2_total_graph, ncol=2)
...
tspag_scc = ggplot(scc_long, aes(x=wave, y=scc_total)) + 
  geom_line() + guides(colour=FALSE) + xlab("Wave") +
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/scc01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
SPPA_Wave1_totals_graph<-ggplot(SPPA_Wave1_totals, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+
  ggtitle(paste0("Self-Perception Profile for Adolescents total subscores for ",length(unique(SPPA_Wave1_outdf$tagid[!is.na(SPPA_Wave1_outdf$SPPA_scholasticcomp_total)]))," participants")) + geom_vline(aes(xintercept = median(SPPA_Wave1_outdf$SPPA_globalselfworth_total, na.rm=T), colour="Global_Self_Worth"))
...
SPPA_Wave2_totals_graph<-ggplot(SPPA_Wave2_totals, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+ 
  ggtitle(paste0("Self-Perception Profile for Adolescents total subscores for ",length(unique(SPPA_Wave2_outdf$tagid[!is.na(SPPA_Wave2_outdf$SPPA_scholasticcomp_total)]))," participants")) + geom_vline(aes(xintercept = median(SPPA_Wave2_outdf$SPPA_globalselfworth_total, na.rm=T), colour="Global_Self_Worth"))
...
SPPA_Wave3_totals_graph<-ggplot(SPPA_Wave3_totals, aes(x=value, colour=item)) +
  geom_density(alpha=.3)+ 
  ggtitle(paste0("Self-Perception Profile for Adolescents total subscores for ",length(unique(SPPA_Wave3_outdf$tagid[!is.na(SPPA_Wave3_outdf$SPPA_scholasticcomp_total)]))," participants")) + geom_vline(aes(xintercept = median(SPPA_Wave3_outdf$SPPA_globalselfworth_total, na.rm=T), colour="Global_Self_Worth"))+ geom_vline(aes(xintercept = median(SPPA_Wave3_outdf$SPPA_romanticappeal_total, na.rm=T), colour="Romantic_Appeal"))


require(gridExtra)
grid.arrange(SPPA_Wave1_totals_graph, SPPA_Wave2_totals_graph, ncol=2)
...
sppa_change <- sppa_change[c("tagid","SPPA_globalselfworth_total.x","SPPA_globalselfworth_total.y")]
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/harterac01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
TAG149_special <- bind_rows(SSDS %>% filter(tagid=="TAG149" & grepl("W2|W3", survey_name)), TAG149_special)
...
SSDS_Wave3_outdf <- SSDS_Wave1 %>% filter(grepl("W3",survey_name)) 
write.csv(SSDS_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave3/SSDS_Wave3.csv"))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/ssds01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/panas_c01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
YRBS_Wave2_outdf <- YRBS_Wave1 %>% filter(grepl("W2",survey_name)) 
write.csv(YRBS_Wave2_outdf, file = paste0(workdir,"Questionnaires/Wave2/YRBS_Wave2.csv"))

YRBS_Wave3_outdf <- YRBS_Wave1 %>% filter(grepl("W3",survey_name)) 
write.csv(YRBS_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave3/YRBS_Wave3.csv"))
```
...
ndar_cbcl_data <- left_join(CBCL_Wave1,distinct(redcap_cleaned, tagid, dob),by="tagid") %>%
  ...
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         ...
         interp_kids=ifelse(interp_kids==3, 999, interp_kids))
...
write.table(together,file =paste0(workdir,"RDoCdb/output/dec2019/cbcl01.csv"),sep=",",na = "",row.names=FALSE,col.names=FALSE)
...
PDS_parent_Wave2_outdf <- PDS_parent_Wave1 %>% filter(grepl("W2",survey_name)) 
write.csv(PDS_parent_Wave2_outdf, file = paste0(workdir,"Questionnaires/Wave2/PDS_parent_Wave2.csv"))

PDS_parent_Wave3_outdf <- PDS_parent_Wave1 %>% filter(grepl("W3",survey_name)) 
write.csv(PDS_parent_Wave3_outdf, file = paste0(workdir,"Questionnaires/Wave3/PDS_parent_Wave3.csv"))
```
