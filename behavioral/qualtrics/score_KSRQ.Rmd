...
KSRQ_Wave1<-left_join(KSRQ,survey_date, by = c("tagid","survey_name")) %>% 
  ...
  mutate(KSRQ_adm_mean=round(rowMeans(cbind(K_SRQ_1,K_SRQ_7,K_SRQ_11,K_SRQ_17), na.rm=T),3),
         ...
         KSRQ_sex_mean=round(rowMeans(cbind(K_SRQ_9,K_SRQ_19), na.rm=T),3), # item 13 up to 5/9/19 was missing, will add new item #23 here (after 5/9/19) to replace item 13 - MLB
...
ndar_ksrq_data <- left_join(KSRQ_Wave1,distinct(redcap_cleaned, tagid, dob),by="tagid") %>%
  ...
  mutate(interview_date=paste0(sprintf("%02d",month(survey_date)),"/",sprintf("%02d",day(survey_date)),"/",year(survey_date)),
         ...
         ksrq_11=K_SRQ_11,
         ksrq_12=K_SRQ_12,
         ksrq_13=-99, # will replace with new item #23 (from qualtrics) here from 5/9/19 onwards - MLB
         ksrq_14=K_SRQ_13,
         ksrq_15=K_SRQ_14,
         ksrq_16=K_SRQ_15,
         ksrq_17=K_SRQ_16,
